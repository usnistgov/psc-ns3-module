...
capabilities for 4G LTE systems introduced in 3GPP Release 12
and later releases, including capabilities for 5G NR systems.
This includes Device-to-Device (D2D) communications over sidelink
and mission-critical push-to-talk (MCPTT) application.

Presently support for public safety communications is distributed among the
following four |ns3| modules:

1) ``psc``:  (this module)  Support for models and scenarios that are specific to public safety communications.
2) ``nr``:  Support for 5G NR sidelink mode 2 communications.
3) ``nr-prose``:  Support for 5G NR ProSe (discovery and UE-to-Network Relay)
4) ``sip``: Session Initiation Protocol (SIP) model.

Documentation for the PSC features implemented in the |ns3| ``nr``, ``nr-prose``, and ``sip`` modules is provided in the respective
module documentation.  This document describes the |ns3| ``psc`` module.

A large portion of the code related to public safety communications is
found in the sidelink implementation in the ``nr`` module and in the ProSe and
UE-to-Network relay implementation in the ``nr-prose`` module.  This is because
the ProSe services of sidelink communications, discovery, and synchronization
are deeply connected to the 5G NR models and difficult to factor into a
separate module.

The ``psc`` module includes:
...
PSC models as well as modifications to the ``antenna``, ``buildings``,
``propagation``, and ``lte`` modules.

Through January 2024, incremental releases were made on this special version
of ns-3, briefly summarized below:
...
v7.0    Jan 10, 2024 Aligned to ns-3.40, several minor updates
======= ============ =========================================================

Starting with this release, v8.0, the PSC module is decoupled from 4G LTE
support and instead is maintained as a standalone module, suitable for
inclusion in future 4G LTE-based releases (with NIST D2D extensions) and
also 5G NR-based releases.
