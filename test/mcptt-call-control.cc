...
class TestCallMachine : public McpttCallMachineGrpBasic
{
  ...
  public:
    ...
    virtual void SetPostRxCb(
        const Callback<void, const TestCallMachine&, const McpttCallMsg&> postRxCb);
    virtual void SetPostTimerExpCb(
        const Callback<void, const TestCallMachine&, const McpttTimer&> timerExpCb);
    virtual void SetPostTxCb(
        const Callback<void, const TestCallMachine&, const McpttCallMsg&> postTxCb);
    virtual void SetPreRxCb(
        const Callback<void, const TestCallMachine&, const McpttCallMsg&> preRxCb);
    virtual void SetPreTimerExpCb(
        const Callback<void, const TestCallMachine&, const McpttTimer&> timerExpCb);
    virtual void SetPreTxCb(
        const Callback<void, const TestCallMachine&, const McpttCallMsg&> preTxCb);
    virtual void SetStartState(Ptr<McpttCallMachineGrpBasicState> startState);
    virtual void SetStateChangeTestCb(
        const Callback<void,
                       const TestCallMachine&,
                       Ptr<McpttCallMachineGrpBasicState>,
                       Ptr<McpttCallMachineGrpBasicState>> stateChangeCb);
};

class McpttCallSetupJoinCall : public McpttTestCase
{
  public:
    McpttCallSetupJoinCall(const std::string& name = "Call Setup - Join Call",
  ...
  protected:
    ...
    virtual void UeAStateChangeCb(const TestCallMachine& machine,
                                  Ptr<McpttCallMachineGrpBasicState> oldState,
                                  Ptr<McpttCallMachineGrpBasicState> newState);
    ...
    virtual void UeBStateChangeCb(const TestCallMachine& machine,
                                  Ptr<McpttCallMachineGrpBasicState> oldState,
                                  Ptr<McpttCallMachineGrpBasicState> newState);
    ...
    virtual void UeCStateChangeCb(const TestCallMachine& machine,
                                  Ptr<McpttCallMachineGrpBasicState> oldState,
                                  Ptr<McpttCallMachineGrpBasicState> newState);
...
{
  public:
    McpttCallSetupNewCall(const std::string& name = "Call Setup - New Call",
                          Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallMerge(const std::string& name = "Call Merge",
                   Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallRelease(const std::string& name = "Call Release",
                     Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallReleaseAfterProbe(const std::string& name = "Call Release - After Probe",
                               Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallReleasePendingUserAction1(
        const std::string& name = "Call Release - Pending User Action 1",
        Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallReleasePendingUserAction2(
        const std::string& name = "Call Release - Pending User Action 2",
        Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallReleaseMaxDuration(const std::string& name = "Call Release - Max Duration",
                                Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallReleaseAndSetup(const std::string& name = "Call Release - And Setup",
                             Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttCallReject(const std::string& name = "Call Reject",
                    Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
void
TestCallMachine::SetPostTimerExpCb(
    const Callback<void, const TestCallMachine&, const McpttTimer&> timerExpCb)
{
...
void
TestCallMachine::SetPreTimerExpCb(
    const Callback<void, const TestCallMachine&, const McpttTimer&> timerExpCb)
{
...
void
TestCallMachine::SetStateChangeTestCb(
    const Callback<void,
                   const TestCallMachine&,
                   Ptr<McpttCallMachineGrpBasicState>,
                   Ptr<McpttCallMachineGrpBasicState>> stateChangeCb)
...
McpttCallSetupJoinCall::McpttCallSetupJoinCall(const std::string& name,
...
McpttCallSetupJoinCall::Configure()
{
    ...
    Ptr<TestCallMachine> ueAMachine =
        DynamicCast<TestCallMachine, McpttCallMachine>(GetCallMachine(0));
    Ptr<TestCallMachine> ueBMachine =
        DynamicCast<TestCallMachine, McpttCallMachine>(GetCallMachine(1));
    Ptr<TestCallMachine> ueCMachine =
        DynamicCast<TestCallMachine, McpttCallMachine>(GetCallMachine(2));
...
McpttCallSetupJoinCall::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS1ToS2,
                          true,
                          "UE A did not make transistion from "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAS2ToS3,
                          true,
                          "UE A did not make transistion from "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << " to "
    ...
    if (msg.IsA(McpttCallMsgGrpAnnoun::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueAS2ToS3,
                              ...
                              "UE A never made transition from "
                                  << McpttCallMachineGrpBasicStateS2::GetStateId() << " to "
                                  << McpttCallMachineGrpBasicStateS3::GetStateId());
        NS_TEST_ASSERT_MSG_EQ(m_ueATxProbe,
                              true,
                              "UE A never sent a " << McpttCallMsgGrpProbe::GetTypeId()
                                                   << " message.");
        NS_TEST_ASSERT_MSG_EQ(m_ueARxAnnoun,
                              false,
                              "Already recieved a " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                    << " message.");
        m_ueARxAnnoun = true;
        ...
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE A not in " << McpttCallMachineGrpBasicStateS3::GetStateId());
...
void
McpttCallSetupJoinCall::UeAStateChangeCb(const TestCallMachine& machine,
                                         Ptr<McpttCallMachineGrpBasicState> oldState,
                                         Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
        ...
        newState->GetInstanceStateId() == McpttCallMachineGrpBasicStateS2::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueATxProbe,
                              true,
                              "UE never sent " << McpttCallMsgGrpProbe::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueARxAnnoun,
                              false,
                              "UE already received " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueAS1ToS2,
                              false,
             ...
             newState->GetInstanceStateId() == McpttCallMachineGrpBasicStateS3::GetStateId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueAS1ToS2,
                              true,
                              "UE A hasn't made transition from "
                                  << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
                                  << McpttCallMachineGrpBasicStateS2::GetStateId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueAS2ToS3,
                              false,
...
McpttCallSetupJoinCall::UeATxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueATxProbe,
                              false,
                              "UE A already sent " << McpttCallMsgGrpProbe::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueARxAnnoun,
                              false,
                              "UE already received " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueAS1ToS2,
...
McpttCallSetupJoinCall::UeBTimerExpCb(const TestCallMachine& machine, const McpttTimer& timer)
{
    ...
    if (timer.GetId() == machine.GetTfg2()->GetId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueBRxProbe,
                              true,
                              "UE B never recieved " << McpttCallMsgGrpProbe::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueBTxAnnoun,
                              true,
                              "UE B never sent " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(), true, "UE B timer TFG6 is not running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE B in state "
                                  << state->GetInstanceStateId() << " but should be in "
                                  << McpttCallMachineGrpBasicStateS3::GetStateId() << ".");
        m_ueBTfg2Exp = true;
...
McpttCallSetupJoinCall::UeBRxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueBRxProbe,
                              false,
                              "UE B already recieved " << McpttCallMsgGrpProbe::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueBTxAnnoun,
                              false,
                              "UE B already sent " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(), true, "UE B timer TFG6 is not running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE B not in state "
...
McpttCallSetupJoinCall::UeBTxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B sent unexpected message: " << msg.GetInstanceTypeId() << ".");
    }
...
void
McpttCallSetupJoinCall::UeBStateChangeCb(const TestCallMachine& machine,
                                         Ptr<McpttCallMachineGrpBasicState> oldState,
                                         Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallSetupJoinCall::UeCTimerExpCb(const TestCallMachine& machine, const McpttTimer& timer)
{
    ...
    if (timer.GetId() == machine.GetTfg2()->GetId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxProbe,
                              true,
                              "UE C never recieved " << McpttCallMsgGrpProbe::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueCTxAnnoun,
                              true,
                              "UE C never sent " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
...
McpttCallSetupJoinCall::UeCRxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxProbe,
                              false,
                              "UE C already recieved " << McpttCallMsgGrpProbe::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueCTxAnnoun,
                              false,
                              "UE C already sent " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(), true, "UE C timer TFG6 is not running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE C not in state "
...
McpttCallSetupJoinCall::UeCTxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C sent unexpected message: " << msg.GetInstanceTypeId() << ".");
    }
...
void
McpttCallSetupJoinCall::UeCStateChangeCb(const TestCallMachine& machine,
                                         Ptr<McpttCallMachineGrpBasicState> oldState,
                                         Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallSetupNewCall::McpttCallSetupNewCall(const std::string& name,
                                             Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
...
McpttCallSetupNewCall::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueARxAcceptCount,
                          2,
                          "UE A recieved " << m_ueARxAcceptCount << " "
                                           << McpttCallMsgGrpAccept::GetTypeId() << " messages.");
    NS_TEST_ASSERT_MSG_EQ(m_ueAS1ToS2,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS2ToS3,
                          ...
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS3::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATfg1ExpCount,
                          1,
                          "UE A TFG1 expired " << m_ueATfg1ExpCount << " time(s).");
    NS_TEST_ASSERT_MSG_EQ(m_ueATfg3ExpCount,
                          3,
                          "UE A TFG1 expired " << m_ueATfg3ExpCount << " time(s).");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxProbeCount,
                          4,
                          "UE A TFG1 expired " << m_ueATxProbeCount << " time(s).");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxAnnoun,
                          true,
                          "UE A never sent send " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                  << " message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxAnnoun,
                          true,
                          "UE B never recieved " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                 << " message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxProbeCount,
                          4,
                          "UE B recieved " << m_ueBRxProbeCount << " "
                                           << McpttCallMsgGrpProbe::GetTypeId() << " messages.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBS1ToS3,
                          true,
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueBTxAccept,
                          true,
                          "UE B never sent " << McpttCallMsgGrpAccept::GetTypeId() << " message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxAnnoun,
                          true,
                          "UE C never recieved " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                 << " message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxProbeCount,
                          4,
                          "UE C recieved " << m_ueCRxProbeCount << " "
                                           << McpttCallMsgGrpProbe::GetTypeId() << " messages.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCS1ToS3,
                          true,
                          "UE C never made transition from "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS3::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCTxAccept,
                          true,
                          "UE C never sent " << McpttCallMsgGrpAccept::GetTypeId() << " message.");
}
...
McpttCallSetupNewCall::UeATimerExpCb(const TestCallMachine& machine, const McpttTimer& timer)
{
    ...
    if (timer.GetId() == machine.GetTfg1()->GetId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueATxProbeCount,
                              4,
                              "UE A sent " << m_ueATxProbeCount << " message(s).");
        NS_TEST_ASSERT_MSG_EQ(m_ueARxAcceptCount,
                              0,
                              "UE A already recieved " << McpttCallMsgGrpAccept::GetTypeId()
                                                       << " message.");
        NS_TEST_ASSERT_MSG_EQ(m_ueAS1ToS2,
                              true,
                              "UE A never made transition from "
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueATfg3ExpCount,
                              3,
                              "UE A timer TFG3 expired " << m_ueATfg3ExpCount << " time(s).");
        NS_TEST_ASSERT_MSG_EQ(m_ueATxAnnoun,
                              true,
                              "UE A never sent " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                 << " message.");
        ...
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE A in state " << state->GetInstanceStateId() << " instead of "
                                               << McpttCallMachineGrpBasicStateS3::GetStateId()
                                               << ".");
        m_ueATfg1ExpCount += 1;
    }
    else if (timer.GetId() == machine.GetTfg3()->GetId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueATfg1ExpCount,
                              0,
                              "UE A timer TFG1 expired " << m_ueATfg1ExpCount << " time(s).");
        NS_TEST_ASSERT_MSG_LT(m_ueATxProbeCount,
                              5,
                              "UE A sent " << m_ueATxProbeCount << " message(s).");
        NS_TEST_ASSERT_MSG_LT(m_ueATfg3ExpCount,
                              4,
                              "UE A timer TFG3 expired " << m_ueATfg3ExpCount << " time(s).");
        NS_TEST_ASSERT_MSG_EQ(m_ueARxAcceptCount,
                              0,
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueATxAnnoun,
                              false,
                              "UE A already sent " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                   << " message.");
...
McpttCallSetupNewCall::UeARxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    {
        NS_TEST_ASSERT_MSG_LT(m_ueARxAcceptCount,
                              2,
                              "UE A recieved " << m_ueARxAcceptCount << " "
...
void
McpttCallSetupNewCall::UeAStateChangeCb(const TestCallMachine& machine,
                                        Ptr<McpttCallMachineGrpBasicState> oldState,
                                        Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
McpttCallSetupNewCall::UeATxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        NS_TEST_ASSERT_MSG_GT_OR_EQ(m_ueATxProbeCount,
                                    m_ueATfg3ExpCount,
                                    "UE A sent " << m_ueATxProbeCount << " message(s).");
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueATfg1ExpCount,
                              0,
                              "UE A timer TFG1 expired " << m_ueATfg1ExpCount << " time(s).");
        NS_TEST_ASSERT_MSG_LT_OR_EQ(m_ueATfg3ExpCount,
                                    m_ueATxProbeCount,
                                    "UE A timer TFG3 expired " << m_ueATfg3ExpCount << " time(s).");
        NS_TEST_ASSERT_MSG_EQ(m_ueATxAnnoun,
                              false,
...
McpttCallSetupNewCall::UeBRxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueBRxAnnoun,
                              false,
                              "UE B already recieved " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                       << " message.");
        NS_TEST_ASSERT_MSG_LT(m_ueBRxProbeCount,
                              4,
                              "UE B recieved " << m_ueBRxProbeCount << " "
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueBTxAccept,
                              false,
                              "UE B already sent " << McpttCallMsgGrpAccept::GetTypeId()
                                                   << " message.");
        NS_TEST_ASSERT_MSG_EQ(tfg1->IsRunning(),
                              false,
                              "UE B timer " << tfg1->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(tfg3->IsRunning(),
                              false,
                              "UE B timer " << tfg3->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(tfg2->IsRunning(),
                              false,
                              "UE B timer " << tfg2->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(),
                              false,
                              "UE B timer " << tfg6->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
    ...
    else if (msg.IsA(McpttCallMsgGrpAnnoun::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueBTxAccept,
                              true,
                              "UE B never sent " << McpttCallMsgGrpAccept::GetTypeId()
                                                 << " message.");
        NS_TEST_ASSERT_MSG_EQ(tfg1->IsRunning(),
                              false,
                              "UE B timer " << tfg1->GetId() << " is running.");
        ...
        NS_TEST_ASSERT_MSG_EQ(tfg2->IsRunning(),
                              true,
                              "UE B timer " << tfg2->GetId() << " is not running.");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(),
                              true,
                              "UE B timer " << tfg6->GetId() << " is not running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE B in state " << state->GetInstanceStateId() << " not "
                                               << McpttCallMachineGrpBasicStateS3::GetStateId()
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B received unexpected " << msg.GetInstanceTypeId()
                                                          << " message.");
    }
...
void
McpttCallSetupNewCall::UeBStateChangeCb(const TestCallMachine& machine,
                                        Ptr<McpttCallMachineGrpBasicState> oldState,
                                        Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
McpttCallSetupNewCall::UeCRxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxAnnoun,
                              false,
                              "UE C already recieved " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                       << " message.");
        NS_TEST_ASSERT_MSG_LT(m_ueCRxProbeCount,
                              4,
                              "UE C recieved " << m_ueCRxProbeCount << " "
                                               << McpttCallMsgGrpProbe::GetTypeId()
                                               << " messages.");
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueCTxAccept,
                              false,
                              "UE C already sent " << McpttCallMsgGrpAccept::GetTypeId()
                                                   << " message.");
        NS_TEST_ASSERT_MSG_EQ(tfg1->IsRunning(),
                              false,
                              "UE C timer " << tfg1->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(tfg3->IsRunning(),
                              false,
                              "UE C timer " << tfg3->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(tfg2->IsRunning(),
                              false,
                              "UE C timer " << tfg2->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(),
                              false,
                              "UE C timer " << tfg6->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS1::GetStateId(),
                              "UE C in state " << state->GetInstanceStateId() << " not "
                                               << McpttCallMachineGrpBasicStateS1::GetStateId()
                                               << ".");
        m_ueCRxProbeCount += 1;
    }
    else if (msg.IsA(McpttCallMsgGrpAnnoun::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueCTxAccept,
                              true,
                              "UE C never sent " << McpttCallMsgGrpAccept::GetTypeId()
                                                 << " message.");
        NS_TEST_ASSERT_MSG_EQ(tfg1->IsRunning(),
                              false,
                              "UE C timer " << tfg1->GetId() << " is running.");
        ...
        NS_TEST_ASSERT_MSG_EQ(tfg2->IsRunning(),
                              true,
                              "UE C timer " << tfg2->GetId() << " is not running.");
        NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(),
                              true,
                              "UE C timer " << tfg6->GetId() << " is not running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE C in state " << state->GetInstanceStateId() << " not "
                                               << McpttCallMachineGrpBasicStateS3::GetStateId()
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C received unexpected " << msg.GetInstanceTypeId()
                                                          << " message.");
    }
...
void
McpttCallSetupNewCall::UeCStateChangeCb(const TestCallMachine& machine,
                                        Ptr<McpttCallMachineGrpBasicState> oldState,
                                        Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
McpttCallMerge::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueCS1ToS2,
                          true,
                          "UE C never made transition from "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCS2ToS3,
                          true,
                          "UE C never made transition from "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS3::GetStateId() << ".");
...
void
McpttCallMerge::UeAStateChangeCb(const TestCallMachine& machine,
                                 Ptr<McpttCallMachineGrpBasicState> oldState,
                                 Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallMerge::UeBRxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpAnnoun::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueBTxAnnoun,
                              false,
                              "UE B already sent " << McpttCallMsgGrpAnnoun::GetTypeId() << ".");
        NS_TEST_ASSERT_MSG_LT(m_ueBRxAnnoun,
                              2,
                              "UE B received too many " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                        << ".");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS3::GetStateId(),
                              "UE B in state "
...
void
McpttCallMerge::UeBStateChangeCb(const TestCallMachine& machine,
                                 Ptr<McpttCallMachineGrpBasicState> oldState,
                                 Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallMerge::UeBTxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B sent unexpected " << msg.GetInstanceTypeId() << " message.");
    }
...
McpttCallMerge::UeCTimerExpCb(const TestCallMachine& machine, const McpttTimer& timer)
{
    ...
    if (timer.GetId() == machine.GetTfg3()->GetId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxAnnoun,
                              false,
                              "UE C already received an announcement message.");
        ...
        if (m_ueCTxProbe > 1)
        {
            NS_TEST_ASSERT_MSG_EQ(m_ueCTxProbe,
                                  m_ueCTfg3Exp + 2,
                                  "UE C did not send " << (m_ueCTfg3Exp + 2) << " probe messages.");
        }
        else
        {
...
McpttCallMerge::UeCTxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        ...
        if (m_ueCTfg3Exp > 0)
        {
            NS_TEST_ASSERT_MSG_EQ(m_ueCTfg3Exp,
                                  m_ueCTxProbe - 1,
                                  "UE C timer TFG3 did not expire " << (m_ueCTxProbe - 1)
                                                                    << " times.");
        }
        else
        {
        ...
        if (m_ueCTxProbe > 0)
        {
            NS_TEST_ASSERT_MSG_EQ(m_ueCTxProbe,
                                  m_ueCTfg3Exp + 1,
                                  "UE C did not send " << (m_ueCTfg3Exp + 1) << " probe messages.");
        }
        else
        {
            NS_TEST_ASSERT_MSG_EQ(m_ueCTxProbe, 0, "UE C did already sent probe message.");
    ...
    else if (msg.IsA(McpttCallMsgGrpAnnoun::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueCTfg3Exp,
                              m_ueCTxProbe - 1,
                              "UE C timer TFG3 did not expire " << (m_ueCTxProbe - 1) << " times.");
        NS_TEST_ASSERT_MSG_EQ(m_ueCTxProbe,
                              m_ueCTfg3Exp + 1,
                              "UE C did not send " << (m_ueCTfg3Exp + 1) << " probe messages.");
        NS_TEST_ASSERT_MSG_EQ(m_ueCTxAnnoun, false, "UE C already sent an announcement message.");
...
void
McpttCallMerge::UeCStateChangeCb(const TestCallMachine& machine,
                                 Ptr<McpttCallMachineGrpBasicState> oldState,
                                 Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
McpttCallRelease::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS3ToS6,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS3::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAS6ToS1,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxAnnoun,
                          true,
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueBTxAnnoun,
                          true,
                          "UE B never sent a " << McpttCallMsgGrpAnnoun::GetTypeId()
                                               << " message.");
}
...
void
McpttCallRelease::UeAStateChangeCb(const TestCallMachine& machine,
                                   Ptr<McpttCallMachineGrpBasicState> oldState,
                                   Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
        ...
        newState->GetInstanceStateId() == McpttCallMachineGrpBasicStateS6::GetStateId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueARxAnnoun,
                              false,
                              "UE A received a " << McpttCallMsgGrpAnnoun::GetTypeId()
                                                 << " message.");
        NS_TEST_ASSERT_MSG_EQ(m_ueATfg5Exp, false, "UE A TFG5 already expired.");
        NS_TEST_ASSERT_MSG_EQ(m_ueAS6ToS1,
                              false,
...
void
McpttCallRelease::UeBStateChangeCb(const TestCallMachine& machine,
                                   Ptr<McpttCallMachineGrpBasicState> oldState,
                                   Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttCallRelease::UeCStateChangeCb(const TestCallMachine& machine,
                                   Ptr<McpttCallMachineGrpBasicState> oldState,
                                   Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallReleaseAfterProbe::McpttCallReleaseAfterProbe(const std::string& name,
                                                       Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueAS2ToS7(false),
      m_ueAS7ToS1(false),
...
McpttCallReleaseAfterProbe::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseAfterProbe::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseAfterProbe::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseAfterProbe::UeCStateChangeCb, this));
...
McpttCallReleaseAfterProbe::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS2ToS7,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS7::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAS7ToS1,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS7::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << ".");
...
void
McpttCallReleaseAfterProbe::UeAStateChangeCb(const TestCallMachine& machine,
                                             Ptr<McpttCallMachineGrpBasicState> oldState,
                                             Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
void
McpttCallReleaseAfterProbe::UeBStateChangeCb(const TestCallMachine& machine,
                                             Ptr<McpttCallMachineGrpBasicState> oldState,
                                             Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttCallReleaseAfterProbe::UeCStateChangeCb(const TestCallMachine& machine,
                                             Ptr<McpttCallMachineGrpBasicState> oldState,
                                             Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallReleasePendingUserAction1::McpttCallReleasePendingUserAction1(
...
McpttCallReleasePendingUserAction1::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleasePendingUserAction1::UeAStateChangeCb, this));
    ueAMachine->SetPostTimerExpCb(
        MakeCallback(&McpttCallReleasePendingUserAction1::UeATimerExpCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleasePendingUserAction1::UeBStateChangeCb, this));
    ueBMachine->SetPostTimerExpCb(
        MakeCallback(&McpttCallReleasePendingUserAction1::UeBTimerExpCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleasePendingUserAction1::UeCStateChangeCb, this));
    ueCMachine->SetPostTimerExpCb(
        MakeCallback(&McpttCallReleasePendingUserAction1::UeCTimerExpCb, this));
...
McpttCallReleasePendingUserAction1::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS4ToS6,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS4::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAS6ToS3,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS3::GetStateId() << ".");
...
void
McpttCallReleasePendingUserAction1::UeATimerExpCb(const TestCallMachine& machine,
...
void
McpttCallReleasePendingUserAction1::UeAStateChangeCb(const TestCallMachine& machine,
                                                     Ptr<McpttCallMachineGrpBasicState> oldState,
                                                     Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
void
McpttCallReleasePendingUserAction1::UeBTimerExpCb(const TestCallMachine& machine,
                                                  const McpttTimer& timer)
{
    NS_TEST_ASSERT_MSG_EQ(false, true, "UE B timer " << timer << " unexpectedly expired.");
}
...
void
McpttCallReleasePendingUserAction1::UeBStateChangeCb(const TestCallMachine& machine,
                                                     Ptr<McpttCallMachineGrpBasicState> oldState,
                                                     Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttCallReleasePendingUserAction1::UeCTimerExpCb(const TestCallMachine& machine,
                                                  const McpttTimer& timer)
{
    NS_TEST_ASSERT_MSG_EQ(false, true, "UE C timer " << timer << " unexpectedly expired.");
}
...
void
McpttCallReleasePendingUserAction1::UeCStateChangeCb(const TestCallMachine& machine,
                                                     Ptr<McpttCallMachineGrpBasicState> oldState,
                                                     Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallReleasePendingUserAction2::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleasePendingUserAction2::UeAStateChangeCb, this));
    ueAMachine->SetPostTimerExpCb(
        MakeCallback(&McpttCallReleasePendingUserAction2::UeATimerExpCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleasePendingUserAction2::UeBStateChangeCb, this));
    ueBMachine->SetPostTimerExpCb(
        MakeCallback(&McpttCallReleasePendingUserAction2::UeBTimerExpCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleasePendingUserAction2::UeCStateChangeCb, this));
    ueCMachine->SetPostTimerExpCb(
        MakeCallback(&McpttCallReleasePendingUserAction2::UeCTimerExpCb, this));
...
McpttCallReleasePendingUserAction2::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS5ToS6,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS4::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
...
void
McpttCallReleasePendingUserAction2::UeATimerExpCb(const TestCallMachine& machine,
                                                  const McpttTimer& timer)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
    Ptr<McpttTimer> tfg2 = machine.GetTfg2();
    ...
    if (timer.GetId() == machine.GetTfg5()->GetId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueAS5ToS6,
                              true,
                              "UE A never made transition from "
                                  << McpttCallMachineGrpBasicStateS5::GetStateId() << " to "
                                  << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
        NS_TEST_ASSERT_MSG_EQ(m_ueAS6ToS1,
                              true,
                              "UE A never made transition from "
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE A timer " << timer.GetId() << " unexpectedly expired.");
    }
...
McpttCallReleasePendingUserAction2::UeARxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE A sent unexpected message: " << msg.GetInstanceTypeId() << ".");
    }
...
void
McpttCallReleasePendingUserAction2::UeAStateChangeCb(const TestCallMachine& machine,
                                                     Ptr<McpttCallMachineGrpBasicState> oldState,
                                                     Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
void
McpttCallReleasePendingUserAction2::UeBTimerExpCb(const TestCallMachine& machine,
                                                  const McpttTimer& timer)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
    Ptr<McpttTimer> tfg2 = machine.GetTfg2();
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B timer " << timer.GetId() << " unexpectedly expired.");
    }
...
void
McpttCallReleasePendingUserAction2::UeBStateChangeCb(const TestCallMachine& machine,
                                                     Ptr<McpttCallMachineGrpBasicState> oldState,
                                                     Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttCallReleasePendingUserAction2::UeCTimerExpCb(const TestCallMachine& machine,
                                                  const McpttTimer& timer)
{
    NS_TEST_ASSERT_MSG_EQ(false, true, "UE C timer " << timer << " unexpectedly expired.");
}
...
void
McpttCallReleasePendingUserAction2::UeCStateChangeCb(const TestCallMachine& machine,
                                                     Ptr<McpttCallMachineGrpBasicState> oldState,
                                                     Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallReleaseMaxDuration::McpttCallReleaseMaxDuration(const std::string& name,
                                                         Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueAS3ToS6(false),
      m_ueAS6ToS1(false),
...
McpttCallReleaseMaxDuration::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseMaxDuration::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseMaxDuration::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseMaxDuration::UeCStateChangeCb, this));
...
void
McpttCallReleaseMaxDuration::UeAStateChangeCb(const TestCallMachine& machine,
                                              Ptr<McpttCallMachineGrpBasicState> oldState,
                                              Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(false,
                              true,
                              "UE A made unexpected state transition to "
...
void
McpttCallReleaseMaxDuration::UeBStateChangeCb(const TestCallMachine& machine,
                                              Ptr<McpttCallMachineGrpBasicState> oldState,
                                              Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttCallReleaseMaxDuration::UeCStateChangeCb(const TestCallMachine& machine,
                                              Ptr<McpttCallMachineGrpBasicState> oldState,
                                              Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallReleaseAndSetup::McpttCallReleaseAndSetup(const std::string& name,
                                                   Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueAS1ToS2(false),
      m_ueAS2ToS7(false),
...
McpttCallReleaseAndSetup::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseAndSetup::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseAndSetup::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttCallReleaseAndSetup::UeCStateChangeCb, this));
...
McpttCallReleaseAndSetup::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAS2ToS7,
                          ...
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS7::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAS7ToS2,
                          true,
                          "UE A never made transition from "
                              << McpttCallMachineGrpBasicStateS7::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS2::GetStateId() << ".");
...
void
McpttCallReleaseAndSetup::UeAStateChangeCb(const TestCallMachine& machine,
                                           Ptr<McpttCallMachineGrpBasicState> oldState,
                                           Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
        ...
        else
        {
            NS_TEST_ASSERT_MSG_EQ(true,
                                  ...
                                  "UE A in state "
                                      ...
                                      << McpttCallMachineGrpBasicStateS1::GetStateId() << " or "
...
void
McpttCallReleaseAndSetup::UeBStateChangeCb(const TestCallMachine& machine,
                                           Ptr<McpttCallMachineGrpBasicState> oldState,
                                           Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttCallReleaseAndSetup::UeCStateChangeCb(const TestCallMachine& machine,
                                           Ptr<McpttCallMachineGrpBasicState> oldState,
                                           Ptr<McpttCallMachineGrpBasicState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttCallReject::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxProbeCount,
                          ...
                          "UE B recieved " << m_ueBRxProbeCount << " "
                                           << McpttCallMsgGrpProbe::GetTypeId() << " messages.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBS1ToS4,
                          true,
                          "UE B never made transition from "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS4::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBS4ToS6,
                          true,
                          "UE B never made transition from "
                              << McpttCallMachineGrpBasicStateS4::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxProbeCount,
                          ...
                          "UE C recieved " << m_ueCRxProbeCount << " "
                                           << McpttCallMsgGrpProbe::GetTypeId() << " messages.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCS1ToS4,
                          true,
                          "UE B never made transition from "
                              << McpttCallMachineGrpBasicStateS1::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS4::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCS4ToS6,
                          true,
                          "UE B never made transition from "
                              << McpttCallMachineGrpBasicStateS4::GetStateId() << " to "
                              << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
...
void
McpttCallReject::UeAStateChangeCb(const TestCallMachine& machine,
                                  Ptr<McpttCallMachineGrpBasicState> oldState,
                                  Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
McpttCallReject::UeBRxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    ...
    if (msg.IsA(McpttCallMsgGrpProbe::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueBS4ToS6,
                              ...
                              "UE B already made transition from "
                                  << McpttCallMachineGrpBasicStateS4::GetStateId() << " to "
                                  << McpttCallMachineGrpBasicStateS6::GetStateId() << ".");
        NS_TEST_ASSERT_MSG_EQ(tfg4->IsRunning(),
                              false,
                              "UE B timer " << tfg4->GetId() << " is running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS1::GetStateId(),
                              "UE B in state " << state->GetInstanceStateId() << " not "
                                               << McpttCallMachineGrpBasicStateS1::GetStateId()
    ...
    else if (msg.IsA(McpttCallMsgGrpAnnoun::GetTypeId()))
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(tfg4->IsRunning(),
                              true,
                              "UE B timer " << tfg4->GetId() << " is not running.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttCallMachineGrpBasicStateS4::GetStateId(),
                              "UE B in state " << state->GetInstanceStateId() << " not "
                                               << McpttCallMachineGrpBasicStateS4::GetStateId()
...
McpttCallReject::UeBTxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    NS_TEST_ASSERT_MSG_EQ(true,
                          false,
                          "UE B received unexpected " << msg.GetInstanceTypeId() << " message.");
}

void
McpttCallReject::UeBStateChangeCb(const TestCallMachine& machine,
                                  Ptr<McpttCallMachineGrpBasicState> oldState,
                                  Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
...
McpttCallReject::UeCTimerExpCb(const TestCallMachine& machine, const McpttTimer& timer)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C timer " << timer.GetId() << " unexpectedly expired.");
    }
    ...
    NS_TEST_ASSERT_MSG_EQ(tfg4->IsRunning(),
                          false,
                          "UE C timer " << tfg4->GetId() << " is running.");
    NS_TEST_ASSERT_MSG_EQ(tfg5->IsRunning(),
                          true,
                          "UE C timer " << tfg5->GetId() << " is not running.");
    NS_TEST_ASSERT_MSG_EQ(tfg6->IsRunning(),
                          false,
                          "UE C timer " << tfg6->GetId() << " is running.");
}
...
McpttCallReject::UeCTxCb(const TestCallMachine& machine, const McpttCallMsg& msg)
{
    NS_TEST_ASSERT_MSG_EQ(true,
                          false,
                          "UE C received unexpected " << msg.GetInstanceTypeId() << " message.");
}

void
McpttCallReject::UeCStateChangeCb(const TestCallMachine& machine,
                                  Ptr<McpttCallMachineGrpBasicState> oldState,
                                  Ptr<McpttCallMachineGrpBasicState> newState)
{
    Ptr<McpttTimer> tfg1 = machine.GetTfg1();
