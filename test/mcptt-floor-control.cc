...
class TestFloorMachine : public McpttOffNetworkFloorParticipant
{
  ...
  public:
    ...
    virtual void SetPostRxCb(
        const Callback<void, const TestFloorMachine&, const McpttMsg&> postRxCb);
    virtual void SetPostTimerExpCb(
        const Callback<void, const TestFloorMachine&, Ptr<McpttTimer>> timerExpCb);
    virtual void SetPostTxCb(
        const Callback<void, const TestFloorMachine&, const McpttMsg&> postTxCb);
    ...
    virtual void SetStateChangeTestCb(
        const Callback<void,
                       const TestFloorMachine&,
                       Ptr<McpttOffNetworkFloorParticipantState>,
                       Ptr<McpttOffNetworkFloorParticipantState>> stateChangeCb);
};
...
{
  public:
    McpttFloorRequestIdle(const std::string& name = "Floor Request - Idle",
  ...
  protected:
    ...
    virtual void StateChangeCb(const TestFloorMachine& machine,
                               Ptr<McpttOffNetworkFloorParticipantState> oldState,
                               Ptr<McpttOffNetworkFloorParticipantState> newState);
    ...
    virtual void UeBStateChangeCb(const TestFloorMachine& machine,
                                  Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                  Ptr<McpttOffNetworkFloorParticipantState> newState);
    ...
    virtual void UeCStateChangeCb(const TestFloorMachine& machine,
                                  Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                  Ptr<McpttOffNetworkFloorParticipantState> newState);
...
{
  public:
    McpttFloorRequestDenied(const std::string& name = "Floor Request - Denied",
                            Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
  ...
  protected:
    ...
    virtual void UeAStateChangeCb(const TestFloorMachine& machine,
                                  Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                  Ptr<McpttOffNetworkFloorParticipantState> newState);
...
{
  public:
    McpttFloorRequestPreemptive(const std::string& name = "Floor Request - Preemptive",
                                Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorRequestQueued(const std::string& name = "Floor Request - Queued",
                            Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttSessionInitNormal(const std::string& name = "Session Initialization - Normal",
                           Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttSessionInitLoss(const std::string& name = "Session Initialization - Message Lost",
                         Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorReleaseNormal(const std::string& name = "Floor Release - Normal",
                            Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorReleaseAfterReq(const std::string& name = "Floor Release - After Request",
                              Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorReleaseDuringGrantWhileQueued(
        const std::string& name = "Floor Release - During Grant While Queued",
        Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorReleaseWhileQueued(const std::string& name = "Floor Release - While Queued",
                                 Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorRequestIdleBis(const std::string& name = "Floor Request - Idle Bis",
                             Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttSessionInitPrivateCall(const std::string& name = "Session Initialization - Private Call",
                                Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
...
{
  public:
    McpttFloorReleasePreemptedFloorArbitrator(
        const std::string& name = "Floor Release - Preemptive Floor Arbitrator",
        Ptr<McpttTestCaseConfig> config = Create<McpttTestCaseConfig>());
      ...
      m_startState(McpttOffNetworkFloorParticipantStateStartStop::GetInstance())
{
    ...
    SetStateChangeTestCb(MakeNullCallback<void,
                                          const TestFloorMachine&,
                                          Ptr<McpttOffNetworkFloorParticipantState>,
                                          Ptr<McpttOffNetworkFloorParticipantState>>());
}
...
TestFloorMachine::ChangeState(Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    ...
    if (!stateChangeCb.IsNull() && oldState->GetInstanceStateId() != newState->GetInstanceStateId())
    {
...
void
TestFloorMachine::SetPostTimerExpCb(
    const Callback<void, const TestFloorMachine&, Ptr<McpttTimer>> timerExpCb)
{
...
void
TestFloorMachine::SetPreTimerExpCb(
    const Callback<void, const TestFloorMachine&, Ptr<McpttTimer>> timerExpCb)
{
...
void
TestFloorMachine::SetStateChangeTestCb(
    const Callback<void,
                   const TestFloorMachine&,
                   Ptr<McpttOffNetworkFloorParticipantState>,
                   Ptr<McpttOffNetworkFloorParticipantState>> stateChangeCb)
...
McpttFloorRequestIdle::McpttFloorRequestIdle(const std::string& name,
...
McpttFloorRequestIdle::Configure()
{
    ...
    Ptr<TestFloorMachine> ueAMachine =
        DynamicCast<TestFloorMachine, McpttFloorParticipant>(GetFloorMachine(0));
    Ptr<TestFloorMachine> ueBMachine =
        DynamicCast<TestFloorMachine, McpttFloorParticipant>(GetFloorMachine(1));
    Ptr<TestFloorMachine> ueCMachine =
        DynamicCast<TestFloorMachine, McpttFloorParticipant>(GetFloorMachine(2));
...
McpttFloorRequestIdle::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_startStopToPendReq,
                          true,
                          "Did not make transition from 'O: start stop' to 'O: pending request'.");
    NS_TEST_ASSERT_MSG_EQ(
        m_pendReqToHasPerm,
        true,
        "Did not make transistion from 'O: pending request' to 'O: has permission'.");
    NS_TEST_ASSERT_MSG_EQ(
        m_ueBSilenceToNoPerm,
        true,
        "UE B did not make transition from 'O: silence' to 'O: has no permission'.");
    NS_TEST_ASSERT_MSG_EQ(
        m_ueCSilenceToNoPerm,
        true,
        "UE C did not make transition from 'O: silence' to 'O: has no permission'.");
}
...
void
McpttFloorRequestIdle::StateChangeCb(const TestFloorMachine& machine,
                                     Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                     Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
        ...
        newState->GetInstanceStateId() == McpttOffNetworkFloorParticipantStatePendReq::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_startStopToPendReq,
                              false,
                              "UE A already made transition from " << *oldState << " to "
                                                                   << *newState << ".");
        m_startStopToPendReq = true;
    }
    else if (oldState->GetInstanceStateId() ==
                 McpttOffNetworkFloorParticipantStatePendReq::GetStateId() &&
             newState->GetInstanceStateId() ==
                 McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() &&
             !m_pendReqToHasPerm)
    {
        NS_TEST_ASSERT_MSG_EQ(
            m_startStopToPendReq,
            true,
            "UE A already made transition from '"
                << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "' to '"
                << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << "'.");
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "Unexpected state transition from " << *oldState << " to "
                                                                  << *newState << ".");
    }
}
...
McpttFloorRequestIdle::TxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msgTypeId == McpttFloorMsgRequest::GetTypeId())
    {
        if (c201Val == 1)
        {
            NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                                  McpttOffNetworkFloorParticipantStateSilence::GetStateId(),
                                  "Floor machine not in '"
                                      << McpttOffNetworkFloorParticipantStatePendReq::GetStateId()
                                      << "' after floor request was sent.");
        }
        else
        {
            NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(
            true,
            false,
            "A message other than a floor request or floor taken message was sent.");
    }
...
void
McpttFloorRequestIdle::UeBStateChangeCb(const TestFloorMachine& machine,
                                        Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                        Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
    ...
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueBSilenceToNoPerm,
                              false,
                              "UE B already made transition from " << *oldState << " to "
                                                                   << *newState << ".");
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B made invalid state transition from " << *oldState << " to "
                                                                         << *newState << ".");
    }
}
...
void
McpttFloorRequestIdle::UeCStateChangeCb(const TestFloorMachine& machine,
                                        Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                        Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
        ...
        newState->GetInstanceStateId() == McpttOffNetworkFloorParticipantStateNoPerm::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCSilenceToNoPerm,
                              false,
                              "UE C already made transition from " << *oldState << " to "
                                                                   << *newState << ".");
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C made invalid state transition from " << *oldState << " to "
                                                                         << *newState << ".");
    }
}

McpttFloorRequestDenied::McpttFloorRequestDenied(const std::string& name,
                                                 Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
...
McpttFloorRequestDenied::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestDenied::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestDenied::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestDenied::UeCStateChangeCb, this));
...
McpttFloorRequestDenied::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(
        m_hasNoPermToPendReq,
        true,
        "Did not make transition from 'O: has no permission' to 'O: pending request'.");
    NS_TEST_ASSERT_MSG_EQ(
        m_pendReqToNoPerm,
        true,
        "Did not make transistion from 'O: pending request' to 'O: has no permission'.");
...
McpttFloorRequestDenied::UeARxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msg.GetInstanceTypeId() == McpttFloorMsgDeny::GetTypeId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(
            state->GetInstanceStateId(),
            McpttOffNetworkFloorParticipantStateNoPerm::GetStateId(),
            "In state " << *state << " but should be in state '"
                        << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << ".");
        NS_TEST_ASSERT_MSG_EQ(t201->IsRunning(), false, "Timer " << *t201 << " is still running.");
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              msg << " received. Only expected one floor deny message.");
    }
...
void
McpttFloorRequestDenied::UeAStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
        ...
        newState->GetInstanceStateId() == McpttOffNetworkFloorParticipantStatePendReq::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_hasNoPermToPendReq,
                              false,
                              "A transition was already made from " << *oldState << " to "
                                                                    << *newState << ".");
        NS_TEST_ASSERT_MSG_EQ(
            m_pendReqToNoPerm,
            false,
            "A transition was already made from '"
                << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << "' to '"
                << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << "'.");
        ...
        m_hasNoPermToPendReq = true;
             ...
             newState->GetInstanceStateId() ==
                 McpttOffNetworkFloorParticipantStateNoPerm::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_pendReqToNoPerm,
                              false,
                              "A transition was already made from " << *oldState << " to "
                                                                    << *newState << ".");
        NS_TEST_ASSERT_MSG_EQ(
            m_hasNoPermToPendReq,
            true,
            "A transition was never made from '"
                << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << "' to '"
                << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << "'.");
...
McpttFloorRequestDenied::UeATxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttOffNetworkFloorParticipantStateNoPerm::GetStateId(),
                              "UE A in invalid state: " << *state << ".");
        NS_TEST_ASSERT_MSG_EQ(m_txOfFloorReq,
                              false,
                              "A floor request message has already been sent.");
        m_txOfFloorReq = true;
        NS_TEST_ASSERT_MSG_EQ(t201->IsRunning(), true, "Timer " << *t201 << " is not running.");
...
McpttFloorRequestDenied::UeBRxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_rxOfFloorReq,
                              false,
                              "More than one floor request message was received.");
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              msg << " received. Only expecting one floor request message.");
    }
...
void
McpttFloorRequestDenied::UeBStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorRequestDenied::UeBTxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              msg << " sent. Only expecting to send one floor deny message.");
    }
...
McpttFloorRequestDenied::UeCRxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msg.GetInstanceTypeId() == McpttFloorMsgDeny::GetTypeId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxOfFloorReq,
                              true,
                              "UE C has not seen a floor request message yet.");
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxOfFloorDeny,
                              false,
                              "UE C saw more than one floor deny message.");
        m_ueCRxOfFloorDeny = true;
        NS_TEST_ASSERT_MSG_EQ(
            state->GetInstanceStateId(),
            McpttOffNetworkFloorParticipantStateNoPerm::GetStateId(),
            "UE C In state " << *state << " but should be in state '"
                             << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << "'.");
        if (m_rxOfFloorDeny)
        {
            Stop();
    ...
    else if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxOfFloorReq,
                              false,
                              "UE C saw more than one floor request message.");
...
void
McpttFloorRequestDenied::UeCStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C made unexpected state transition from " << *oldState << " to "
                                                                            << *newState << ".");
    }
}
...
McpttFloorRequestPreemptive::McpttFloorRequestPreemptive(const std::string& name,
                                                         Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
...
McpttFloorRequestPreemptive::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestPreemptive::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestPreemptive::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestPreemptive::UeCStateChangeCb, this));
...
McpttFloorRequestPreemptive::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(
        m_ueANoPermToPendReq,
        true,
        "UE A did not make transition from 'O: has no permission' to 'O: pending request'.");
    NS_TEST_ASSERT_MSG_EQ(
        m_ueAPendReqToHasPerm,
        true,
        "UE A did not make transition from 'O: pending request' to 'O: has permission'.");
    ...
    NS_TEST_ASSERT_MSG_EQ(
        m_ueBHasPermToPendGrant,
        true,
        "UE B did not make transition from 'O: has permission' to 'O: pending grant'.");
    NS_TEST_ASSERT_MSG_EQ(
        m_ueBPendGrantToNoPerm,
        true,
        "UE B did not make transition from 'O: pending grant' to 'O: has no permission'.");
...
McpttFloorRequestPreemptive::UeARxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    {
        m_ueARxRtp = true;
        bool validState = (state->GetInstanceStateId() ==
                           McpttOffNetworkFloorParticipantStateNoPerm::GetStateId());

        if (!validState && state->GetInstanceStateId() ==
                               McpttOffNetworkFloorParticipantStatePendReq::GetStateId())
        {
            validState = true;
            NS_TEST_ASSERT_MSG_EQ(t201->IsRunning(), true, "Timer " << *t201 << " is not running.");
    ...
    else if (msg.GetInstanceTypeId() == McpttFloorMsgGranted::GetTypeId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(
            state->GetInstanceStateId(),
            McpttOffNetworkFloorParticipantStateHasPerm::GetStateId(),
            "Not in '" << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << "' state.");
        NS_TEST_ASSERT_MSG_EQ(t201->IsRunning(), false, "Timer " << *t201 << " is still running.");
...
void
McpttFloorRequestPreemptive::UeAStateChangeCb(const TestFloorMachine& machine,
                                              Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                              Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
        ...
        newState->GetInstanceStateId() == McpttOffNetworkFloorParticipantStatePendReq::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueANoPermToPendReq,
                              false,
                              "Invalid sequence of state transitions.");
                 ...
                 McpttOffNetworkFloorParticipantStateHasPerm::GetStateId())
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(m_ueAPendReqToHasPerm,
                              false,
                              "Invalid sequence of state transitions.");
...
void
McpttFloorRequestPreemptive::UeBStateChangeCb(const TestFloorMachine& machine,
                                              Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                              Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
    ...
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueBHasPermToPendGrant,
                              false,
                              "Invalid sequence of state transitions.");
                 ...
                 McpttOffNetworkFloorParticipantStateNoPerm::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueBHasPermToPendGrant,
                              true,
                              "Invalid sequence of state transitions.");
        NS_TEST_ASSERT_MSG_EQ(m_ueBPendGrantToNoPerm,
                              false,
                              "Invalid sequence of state transitions.");
...
void
McpttFloorRequestPreemptive::UeCStateChangeCb(const TestFloorMachine& machine,
                                              Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                              Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorRequestQueued::McpttFloorRequestQueued(const std::string& name,
                                                 Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueANoPermToPendReq(false),
...
McpttFloorRequestQueued::Configure()
{
    ...
    Ptr<McpttMsgDropperImpl> ueAPttAppMcpttMsgDropper =
        Create<McpttMsgDropperImpl>(3, McpttFloorMsgGranted::GetTypeId());
    Ptr<McpttMsgDropperImpl> ueBPttAppMcpttMsgDropper =
        Create<McpttMsgDropperImpl>(5, McpttFloorMsgRequest::GetTypeId());
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestQueued::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestQueued::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestQueued::UeCStateChangeCb, this));
    ueCMachine->SetCurrentSsrc(ueBMachine->GetTxSsrc());
    ...
    ueBPusher->SetAttribute("PttDurationVariable",
                            StringValue("ns3::ConstantRandomVariable[Constant=0.90]"));
...
McpttFloorRequestQueued::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(
        ...
        true,
        "UE A did not make transition from 'O: has no permission' to 'O: pending request'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueAPendReqToQueued,
                          true,
                          "UE A did not make transition from 'O: pending request' to 'O: queued'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueAQueuedToHasPerm,
                          true,
                          "UE A did not make transition from 'O: queued' to 'O: has permission'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxGrant,
                          true,
                          "UE A did not send receive a 'Floor Granted' message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxInfo,
                          true,
                          "UE A did not receive a 'Queue Position Info' message.");
...
void
McpttFloorRequestQueued::UeARxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
...
void
McpttFloorRequestQueued::UeAStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE A made invalid state transition from " << *oldState << " to "
                                                                         << *newState << ".");
    }
}
...
McpttFloorRequestQueued::UeATimerExpCb(const TestFloorMachine& machine, Ptr<McpttTimer> timer)
{
    ...
    {
        m_ueAT201ExpCt += 1;
        NS_TEST_ASSERT_MSG_LT(c201->GetValue(),
                              c201->GetLimit(),
                              "The counter C201 is going to exceeded it's limit.");
    }
    else
    {
...
McpttFloorRequestQueued::UeBRxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueBTxGrant,
                              true,
                              "UE B has not sent a 'Floor Granted' message yet.");
        m_ueBRxRtp = true;
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttOffNetworkFloorParticipantStateNoPerm::GetStateId(),
                              "UE B in invalid state: " << *state << ".");
...
void
McpttFloorRequestQueued::UeBStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
                 ...
                 McpttOffNetworkFloorParticipantStateNoPerm::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(
            m_ueBHasPermToPendGrant,
            true,
            "UE B has not transition from 'O: has permission' to 'O: pending grant'.");
        m_ueBPendGrantToNoPerm = true;
    }
    else if (oldState->GetInstanceStateId() ==
                 McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() &&
...
McpttFloorRequestQueued::UeBTimerExpCb(const TestFloorMachine& machine, Ptr<McpttTimer> timer)
{
    ...
    {
        m_ueBT205ExpCt += 1;
        NS_TEST_ASSERT_MSG_LT(c205->GetValue(),
                              c205->GetLimit(),
                              "The counter C205 is going to reach it's limit.");
    }
    else
    {
...
McpttFloorRequestQueued::UeCRxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msg.GetInstanceTypeId() == McpttFloorMsgGranted::GetTypeId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxInfo,
                              true,
                              "UE C has not seen a queue position info message yet.");
        m_ueCRxGrant = true;
        NS_TEST_ASSERT_MSG_EQ(
            state->GetInstanceStateId(),
            McpttOffNetworkFloorParticipantStateNoPerm::GetStateId(),
    ...
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxReq,
                              true,
                              "UE C has not seen a received request message yet.");
        m_ueCRxInfo = true;
        NS_TEST_ASSERT_MSG_EQ(
            state->GetInstanceStateId(),
            McpttOffNetworkFloorParticipantStateNoPerm::GetStateId(),
...
void
McpttFloorRequestQueued::UeCStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttSessionInitNormal::McpttSessionInitNormal(const std::string& name,
                                               Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueATxGrant(false),
...
McpttSessionInitNormal::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAStartStopToHasPerm,
                          true,
                          "UE A never made the transition from '"
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxGrant, true, "UE B never saw the grant message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBStartStopToSilence,
                          true,
                          "UE B never made the transition from '"
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBSilenceToNoPerm,
                          ...
                          "UE B never made the transition from '"
                              ...
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxGrant, true, "UE C never saw the grant message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCStartStopToSilence,
                          true,
                          "UE C never made the transition from '"
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCSilenceToNoPerm,
...
void
McpttSessionInitNormal::UeAStateChangeCb(const TestFloorMachine& machine,
                                         Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                         Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE A made unexpected state transition from " << *oldState << " to "
                                                                            << *newState << ".");
    }
...
void
McpttSessionInitNormal::UeBStateChangeCb(const TestFloorMachine& machine,
                                         Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                         Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
    ...
    {
        NS_TEST_ASSERT_MSG_EQ(
            m_ueBStartStopToSilence,
            true,
            "UE B never made transition from '"
                << McpttOffNetworkFloorParticipantStateStartStop::GetStateId() << "' to '"
                << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "'.");
        NS_TEST_ASSERT_MSG_EQ(m_ueBSilenceToNoPerm,
                              false,
                              "UE B already made transition from " << *oldState << " to "
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B made unexpected state transition from " << *oldState << " to "
                                                                            << *newState << ".");
    }
...
void
McpttSessionInitNormal::UeCStateChangeCb(const TestFloorMachine& machine,
                                         Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                         Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
McpttSessionInitLoss::Configure()
{
    ...
    Ptr<McpttMsgDropperImpl> ueBPttAppDropper =
        Create<McpttMsgDropperImpl>(2, McpttFloorMsgGranted::GetTypeId());
    Ptr<McpttMsgDropperImpl> ueCPttAppDropper =
        Create<McpttMsgDropperImpl>(2, McpttFloorMsgGranted::GetTypeId());
...
void
McpttSessionInitLoss::UeAStateChangeCb(const TestFloorMachine& machine,
                                       Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                       Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
void
McpttSessionInitLoss::UeBStateChangeCb(const TestFloorMachine& machine,
                                       Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                       Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
void
McpttSessionInitLoss::UeCStateChangeCb(const TestFloorMachine& machine,
                                       Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                       Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
McpttFloorReleaseNormal::McpttFloorReleaseNormal(const std::string& name,
                                                 Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
...
McpttFloorReleaseNormal::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseNormal::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseNormal::UeBStateChangeCb, this));
...
McpttFloorReleaseNormal::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAHasPermToSilence,
                          true,
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueBNoPermToSilence,
                          true,
                          "UE B never made transition from '"
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId()
                              << "' to '"
...
void
McpttFloorReleaseNormal::UeAStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
void
McpttFloorReleaseNormal::UeBStateChangeCb(const TestFloorMachine& machine,
                                          Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                          Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
McpttFloorReleaseAfterReq::McpttFloorReleaseAfterReq(const std::string& name,
                                                     Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueATxRelease(false),
      m_ueARxRtp(false),
...
McpttFloorReleaseAfterReq::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseAfterReq::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseAfterReq::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseAfterReq::UeCStateChangeCb, this));

    ueAMachine->SetCurrentSsrc(ueBMachine->GetTxSsrc());
    ueBMachine->SetCurrentSsrc(ueBMachine->GetTxSsrc());
    ueCMachine->SetCurrentSsrc(ueBMachine->GetTxSsrc());

    McpttQueuedUserInfo userInfo = McpttQueuedUserInfo(ueAId,
                                                       McpttFloorMsgFieldQueuedUserId(ueAId),
                                                       McpttFloorMsgFieldQueuePositionInfo(1, 7));
...
McpttFloorReleaseAfterReq::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(ueBMachineQueue->GetCount(),
                          0,
                          "UE B did not remove UE a from the queue.");

    Simulator::Destroy();

    NS_TEST_ASSERT_MSG_EQ(m_ueATxRelease, true, "UE A never sent release message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxRtp, true, "UE A never received RTP media.");
    NS_TEST_ASSERT_MSG_EQ(m_ueAPendReqToSilence,
                          true,
                          "UE A never made transition from '"
                              << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId()
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueASilenceToNoPerm,
                          true,
                          "UE A never made transition from '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId()
                              << "' to '"
...
McpttFloorReleaseAfterReq::UeARxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    if (msg.GetInstanceTypeId() == McpttMediaMsg::GetTypeId())
    {
        ...
        else if (state->GetInstanceStateId() ==
                 McpttOffNetworkFloorParticipantStatePendReq::GetStateId())
        {
            // Do nothing.
            NS_TEST_ASSERT_MSG_EQ(t201->IsRunning(), true, "Timer " << *t201 << " is not running.");
...
void
McpttFloorReleaseAfterReq::UeAStateChangeCb(const TestFloorMachine& machine,
                                            Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                            Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
void
McpttFloorReleaseAfterReq::UeBStateChangeCb(const TestFloorMachine& machine,
                                            Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                            Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
void
McpttFloorReleaseAfterReq::UeCStateChangeCb(const TestFloorMachine& machine,
                                            Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                            Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorReleaseDuringGrantWhileQueued::Configure()
{
    ...
    ueAMachine->SetPostTimerExpCb(
        MakeCallback(&McpttFloorReleaseDuringGrantWhileQueued::UeATimerExpCb, this));
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseDuringGrantWhileQueued::UeAStateChangeCb, this));
    ...
    ueBMachine->SetPostTimerExpCb(
        MakeCallback(&McpttFloorReleaseDuringGrantWhileQueued::UeBTimerExpCb, this));
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseDuringGrantWhileQueued::UeBStateChangeCb, this));
    ...
    ueCMachine->SetPostTimerExpCb(
        MakeCallback(&McpttFloorReleaseDuringGrantWhileQueued::UeCTimerExpCb, this));
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseDuringGrantWhileQueued::UeCStateChangeCb, this));
...
McpttFloorReleaseDuringGrantWhileQueued::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueATxRelease, true, "UE A never sent release message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueAQueuedToNoPerm,
                          true,
                          "UE A never made transition from '"
                              << McpttOffNetworkFloorParticipantStateQueued::GetStateId()
                              << "' to '"
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueANoPermToSilence,
                          true,
                          "UE A never made transition from '"
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId()
                              << "' to '"
    ...
    NS_TEST_ASSERT_MSG_EQ(
        ...
        true,
        "UE B never made transition from '"
            << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << "' to '"
            << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxRelease, true, "UE B never received a release message.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBPendGrantToSilence,
                          true,
                          "UE B never made transition from '"
                              << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId()
                              << "' to '"
    ...
    NS_TEST_ASSERT_MSG_EQ(
        m_ueCNoPermToSilence,
        2,
        "UE C never made transition from '"
            << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << "' to '"
            << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "' 2 times.");
    NS_TEST_ASSERT_MSG_EQ(m_ueCSilenceToNoPerm,
                          true,
                          "UE C never made transition from '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId()
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeATimerExpCb(const TestFloorMachine& machine,
                                                       Ptr<McpttTimer> timer)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeARxCb(const TestFloorMachine& machine,
                                                 const McpttMsg& msg)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeAStateChangeCb(
    const TestFloorMachine& machine,
    Ptr<McpttOffNetworkFloorParticipantState> oldState,
    Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
        ...
        newState->GetInstanceStateId() == McpttOffNetworkFloorParticipantStateNoPerm::GetStateId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueAQueuedToNoPerm,
                              false,
                              "UE A already made transition from '"
                                  << oldState->GetInstanceStateId() << "' to '"
                                  << newState->GetInstanceStateId() << "'.");
        NS_TEST_ASSERT_MSG_EQ(m_ueATxRelease, true, "UE A never sent release message.");
        NS_TEST_ASSERT_MSG_EQ(t203->IsRunning(), true, "Timer " << *t203 << " is not running.");
        NS_TEST_ASSERT_MSG_EQ(t230->IsRunning(), false, "Timer " << *t230 << " is running.");
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeATxCb(const TestFloorMachine& machine,
                                                 const McpttMsg& msg)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeBRxCb(const TestFloorMachine& machine,
                                                 const McpttMsg& msg)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
    ...
    if (msg.GetInstanceTypeId() == McpttFloorMsgRelease::GetTypeId())
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueBRxRelease,
                              false,
                              "UE B already received floor release message.");
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeBStateChangeCb(
    const TestFloorMachine& machine,
    Ptr<McpttOffNetworkFloorParticipantState> oldState,
    Ptr<McpttOffNetworkFloorParticipantState> newState)
{
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeBTimerExpCb(const TestFloorMachine& machine,
                                                       Ptr<McpttTimer> timer)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
    ...
    if (timer == machine.GetT205())
    {
        if (m_ueBExpT205 == 1)
        {
            NS_TEST_ASSERT_MSG_EQ(m_ueBRxRelease,
                                  true,
                                  "UE B never received floor release message.");
            NS_TEST_ASSERT_MSG_EQ(c205->GetValue(), 1, "Counter C205 is not one.");
            NS_TEST_ASSERT_MSG_EQ(t230->IsRunning(), true, "Timer " << *t230 << " is not running.");
            NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                                  McpttOffNetworkFloorParticipantStateSilence::GetStateId(),
        ...
        {
            NS_TEST_ASSERT_MSG_EQ(m_ueBExpT205,
                                  c205->GetValue() - 2,
                                  "UE B timer " << *timer << " expired too many times.");
            NS_TEST_ASSERT_MSG_EQ(m_ueBRxRelease,
                                  true,
                                  "UE B never received floor release message.");
            NS_TEST_ASSERT_MSG_EQ(t205->IsRunning(), true, "Timer " << *t205 << " is not running.");
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeBTxCb(const TestFloorMachine& machine,
                                                 const McpttMsg& msg)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeCTimerExpCb(const TestFloorMachine& machine,
                                                       Ptr<McpttTimer> timer)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeCRxCb(const TestFloorMachine& machine,
                                                 const McpttMsg& msg)
{
    Ptr<McpttOffNetworkFloorParticipantState> state = machine.GetState();
    ...
    {
        NS_TEST_ASSERT_MSG_EQ(m_ueCRxRelease,
                              false,
                              "UE C already received floor release message.");
        NS_TEST_ASSERT_MSG_EQ(state->GetInstanceStateId(),
                              McpttOffNetworkFloorParticipantStateSilence::GetStateId(),
                              "UE C in invalid state: " << *state << ".");
        NS_TEST_ASSERT_MSG_EQ(t203->IsRunning(), false, "Timer " << *t203 << " is running.");
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeCStateChangeCb(
    const TestFloorMachine& machine,
    Ptr<McpttOffNetworkFloorParticipantState> oldState,
    Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    ...
    if (oldState->GetInstanceStateId() ==
        ...
        newState->GetInstanceStateId() == McpttOffNetworkFloorParticipantStateSilence::GetStateId())
    {
        NS_TEST_ASSERT_MSG_LT(m_ueCNoPermToSilence,
                              2,
                              "UE C made too many transitions from '"
                                  << oldState->GetInstanceStateId() << "' to '"
                                  << newState->GetInstanceStateId() << "'.");
...
void
McpttFloorReleaseDuringGrantWhileQueued::UeCTxCb(const TestFloorMachine& machine,
                                                 const McpttMsg& msg)
{
    NS_TEST_ASSERT_MSG_EQ(true, false, "UE C unexpectedly sent a " << msg << " message.");
}

McpttFloorReleaseWhileQueued::McpttFloorReleaseWhileQueued(const std::string& name,
                                                           Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueATxRelease(false),
      m_ueAQueuedToNoPerm(false),
...
McpttFloorReleaseWhileQueued::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseWhileQueued::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseWhileQueued::UeBStateChangeCb, this));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleaseWhileQueued::UeCStateChangeCb, this));
...
void
McpttFloorReleaseWhileQueued::UeAStateChangeCb(const TestFloorMachine& machine,
                                               Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                               Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    Ptr<McpttTimer> t201 = machine.GetT201();
...
void
McpttFloorReleaseWhileQueued::UeBStateChangeCb(const TestFloorMachine& machine,
                                               Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                               Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
void
McpttFloorReleaseWhileQueued::UeCStateChangeCb(const TestFloorMachine& machine,
                                               Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                               Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    NS_TEST_ASSERT_MSG_EQ(true,
...
McpttFloorRequestIdleBis::McpttFloorRequestIdleBis(const std::string& name,
                                                   Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
...
McpttFloorRequestIdleBis::Configure()
{
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestIdleBis::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestIdleBis::UeBStateChangeCb, this));

    ueBT230->SetDelay(Seconds(0.050));

    ueCPttApp->SetFloorGrantedCb(
        (MakeCallback(&McpttFloorRequestIdleBis::UeCFloorGrantedCb, this)));
    ...
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorRequestIdleBis::UeCStateChangeCb, this));
    ...
    Simulator::Schedule(Seconds(3.0),
                        &McpttFloorParticipant::SendFloorQueuePositionRequest,
                        ueCMachine);
...
McpttFloorRequestIdleBis::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAHasPermToPendGrant,
                          true,
                          "UE A never made transition from "
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId()
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAPendReqToHasPerm,
                          ...
                          "UE A never made transition from "
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxQueuePosInfoReq,
                          true,
                          "UE A never received "
                              << McpttFloorMsgQueuePositionInfo::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxReq,
                          3,
                          "UE A never received 3 " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueASilenceToStartStop,
                          true,
                          "UE A never made transition from "
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAStartStopToPendReq,
                          true,
                          "UE A never made transition from "
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << " to " << McpttOffNetworkFloorParticipantStatePendReq::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxGranted,
                          true,
                          "UE A never sent 2 " << McpttFloorMsgGranted::GetTypeId().GetName()
                                               << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxReq,
                          3,
                          "UE A never sent 3 " << McpttFloorMsgRequest::GetTypeId().GetName()
                                               << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxTaken,
                          true,
                          "UE A never sent " << McpttFloorMsgRequest::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxQueuePosInfo,
                          2,
                          "UE A never sent 2 "
                              << McpttFloorMsgQueuePositionInfo::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxGranted,
                          true,
                          "UE B never received " << McpttFloorMsgGranted::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxQueuePosInfo,
                          2,
                          "UE B never received 2 "
                              << McpttFloorMsgQueuePositionInfo::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxReq,
                          6,
                          "UE B never received 6 " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBSilenceToStartStop,
                          true,
                          "UE B never made transition from "
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBStartStopToNoPerm,
                          true,
                          "UE B never made transition from "
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << " to " << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxQueuePosInfoReq,
                          true,
                          "UE B never received "
                              << McpttFloorMsgQueuePositionRequest::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxTaken,
                          true,
                          "UE B never received " << McpttFloorMsgTaken::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxRtp,
                          2,
                          "UE B received more than 2 " << McpttMediaMsg::GetTypeId().GetName()
                                                       << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCPendReqToQueued,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateQueued::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCQueuedToSilence,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStateQueued::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxGranted,
                          true,
                          "UE C never received " << McpttFloorMsgGranted::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxQueuePosInfo,
                          2,
                          "UE C never received 2 "
                              << McpttFloorMsgQueuePositionInfo::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxReq,
                          3,
                          "UE C never received 3 " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxRtp,
                          2,
                          "UE C never received 2 " << McpttMediaMsg::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxTaken,
                          true,
                          "UE C never received " << McpttFloorMsgTaken::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCStartStopToPendReq,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << " to " << McpttOffNetworkFloorParticipantStatePendReq::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCSilenceToStartStop,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateStartStop::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCTxReq,
                          3,
                          "UE C never sent 3 " << McpttFloorMsgRequest::GetTypeId().GetName()
                                               << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCTxQueuePosInfoReq,
                          true,
                          "UE C never sent "
                              << McpttFloorMsgQueuePositionRequest::GetTypeId().GetName() << ".");
}
...
McpttFloorRequestIdleBis::UeARxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE A received unexpected " << msg.GetInstanceTypeId().GetName()
                                                          << ".");
    }
}

void
McpttFloorRequestIdleBis::UeAStateChangeCb(const TestFloorMachine& machine,
                                           Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                           Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorRequestIdleBis::UeATxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE A sent unexpected " << msg.GetInstanceTypeId() << ".");
    }
}
...
McpttFloorRequestIdleBis::UeBRxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B received unexpected " << msg.GetInstanceTypeId().GetName()
                                                          << ".");
    }
}

void
McpttFloorRequestIdleBis::UeBStateChangeCb(const TestFloorMachine& machine,
                                           Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                           Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorRequestIdleBis::UeCRxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C received unexpected " << msg.GetInstanceTypeId().GetName()
                                                          << ".");
    }
}

void
McpttFloorRequestIdleBis::UeCStateChangeCb(const TestFloorMachine& machine,
                                           Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                           Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorRequestIdleBis::UeCTxCb(const TestFloorMachine& machine, const McpttMsg& msg)
{
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE C sent unexpected " << msg.GetInstanceTypeId() << ".");
    }
}

McpttSessionInitPrivateCall::McpttSessionInitPrivateCall(const std::string& name,
                                                         Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueAHasPermToSilence(false),
      m_ueARxReq(false),
...
McpttSessionInitPrivateCall::Configure()
{
    ...
    Ptr<McpttCallMachineNull> ueACallMachine = DynamicCast<McpttCallMachineNull, McpttCallMachine>(
        ueAMachine->GetCall()->GetCallMachine());
    Ptr<McpttCallMachineNull> ueBCallMachine = DynamicCast<McpttCallMachineNull, McpttCallMachine>(
        ueBMachine->GetCall()->GetCallMachine());
    ...
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttSessionInitPrivateCall::UeAStateChangeCb, this));
    ...
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttSessionInitPrivateCall::UeBStateChangeCb, this));
...
McpttSessionInitPrivateCall::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueARxReq,
                          true,
                          "UE A never received " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(
        m_ueASilenceToPendGrant,
        true,
        "UE A never made the transition from '"
            << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "' to '"
            << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueAStartStopToHasPerm,
                          ...
                          "UE A never made the transition from '"
                              ...
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxGrant,
                          2,
                          "UE A never sent 2 " << McpttFloorMsgGranted::GetTypeId().GetName()
                                               << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxRelease,
                          true,
                          "UE A never sent " << McpttFloorMsgRelease::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBNoPermToSilence,
                          true,
                          "UE B never made the transition from '"
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId()
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBPendReqToHasPerm,
                          true,
                          "UE B never made the transition from '"
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId()
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBSilenceToPendReq,
                          true,
                          "UE B never made the transition from '"
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId()
                              << "' to '"
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueBStartStopToNoPerm,
                          ...
                          "UE B never made the transition from '"
                              ...
                              << "' to '"
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << "'.");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxGrant,
                          2,
                          "UE B never received 2 " << McpttFloorMsgGranted::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxRtp,
                          true,
                          "UE B never received " << McpttMediaMsg::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxRelease,
                          true,
                          "UE B never received " << McpttFloorMsgRelease::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBTxReq,
                          true,
                          "UE B never sent " << McpttFloorMsgRequest::GetTypeId().GetName() << ".");
}
...
void
McpttSessionInitPrivateCall::UeAStateChangeCb(const TestFloorMachine& machine,
                                              Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                              Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
void
McpttSessionInitPrivateCall::UeBStateChangeCb(const TestFloorMachine& machine,
                                              Ptr<McpttOffNetworkFloorParticipantState> oldState,
                                              Ptr<McpttOffNetworkFloorParticipantState> newState)
{
    if (oldState->GetInstanceStateId() ==
...
McpttFloorReleasePreemptedFloorArbitrator::McpttFloorReleasePreemptedFloorArbitrator(
    const std::string& name,
    Ptr<McpttTestCaseConfig> config)
    : McpttTestCase(name, config),
      m_ueAHasPermToPendGrant(false),
...
McpttFloorReleasePreemptedFloorArbitrator::Configure()
{
    ...
    ueAMachine->SetAttribute("GenMedia", BooleanValue(false));
    ueAMachine->SetStartState(McpttOffNetworkFloorParticipantStateHasPerm::GetInstance());
    ueAMachine->SetPostTxCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeATxCb, this));
    ueAMachine->SetPreRxCb(MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeARxCb, this));
    ueAMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeAStateChangeCb, this));
    ueAMachine->SetCurrentSsrc(ueAMachine->GetTxSsrc());
    ...
    ueBMachine->SetAttribute("GenMedia", BooleanValue(false));
    ueBMachine->SetStartState(McpttOffNetworkFloorParticipantStateNoPerm::GetInstance());
    ueBMachine->SetPostRxCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeBRxCb, this));
    ueBMachine->SetPostTxCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeBTxCb, this));
    ueBMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeBStateChangeCb, this));
    ueBMachine->SetCurrentSsrc(ueAMachine->GetTxSsrc());
    ...
    ueCMachine->SetAttribute("GenMedia", BooleanValue(false));
    ueCMachine->SetStartState(McpttOffNetworkFloorParticipantStateNoPerm::GetInstance());
    ueCMachine->SetPostRxCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeCRxCb, this));
    ueCMachine->SetPostTxCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeCTxCb, this));
    ueCMachine->SetStateChangeTestCb(
        MakeCallback(&McpttFloorReleasePreemptedFloorArbitrator::UeCStateChangeCb, this));
...
McpttFloorReleasePreemptedFloorArbitrator::Execute()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueAHasPermToPendGrant,
                          ...
                          "UE A never made transition from "
                              ...
                              << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueAPendGrantToSilence,
                          true,
                          "UE A never made transition from "
                              << McpttOffNetworkFloorParticipantStatePendGrant::GetStateId()
                              << " to " << McpttOffNetworkFloorParticipantStateSilence::GetStateId()
                              << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxDeny,
                          true,
                          "UE A never received " << McpttFloorMsgDeny::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxRelease,
                          true,
                          "UE A never received " << McpttFloorMsgRelease::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueARxReq,
                          2,
                          "UE A never received 2 " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxDeny,
                          true,
                          "UE A never sent " << McpttFloorMsgDeny::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueATxGrant,
                          4,
                          "UE A never sent 4 " << McpttFloorMsgGranted::GetTypeId().GetName()
                                               << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBNoPermToPendReq,
                          true,
                          "UE B never made transition from "
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBPendReqToHasPerm,
                          ...
                          "UE B never made transition from "
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBHasPermToSilence,
                          true,
                          "UE B never made transition from "
                              << McpttOffNetworkFloorParticipantStateHasPerm::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxDeny,
                          true,
                          "UE B never received " << McpttFloorMsgDeny::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxGrant,
                          4,
                          "UE B never received 4 " << McpttFloorMsgGranted::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBRxReq,
                          true,
                          "UE B never received " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBTxDeny,
                          true,
                          "UE B never sent " << McpttFloorMsgDeny::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBTxReq,
                          true,
                          "UE B never sent " << McpttFloorMsgRequest::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueBTxRelease,
                          true,
                          "UE B never sent " << McpttFloorMsgRelease::GetTypeId().GetName() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCNoPermToPendReq,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCPendReqToNoPerm,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStatePendReq::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCNoPermToSilence,
                          true,
                          "UE C never made transition from "
                              << McpttOffNetworkFloorParticipantStateNoPerm::GetStateId() << " to "
                              << McpttOffNetworkFloorParticipantStateSilence::GetStateId() << ".");
    ...
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxRelease,
                          ...
                          "UE A never received " << McpttFloorMsgRelease::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxDeny,
                          2,
                          "UE C never received 2 " << McpttFloorMsgDeny::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxReq,
                          true,
                          "UE C never received " << McpttFloorMsgRequest::GetTypeId().GetName()
                                                 << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCRxGrant,
                          4,
                          "UE C never received 4 " << McpttFloorMsgGranted::GetTypeId().GetName()
                                                   << ".");
    NS_TEST_ASSERT_MSG_EQ(m_ueCTxReq,
                          true,
                          "UE C never sent " << McpttFloorMsgRequest::GetTypeId().GetName() << ".");
}

void
McpttFloorReleasePreemptedFloorArbitrator::UeARxCb(const TestFloorMachine& machine,
                                                   const McpttMsg& msg)
{
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeAStateChangeCb(
    const TestFloorMachine& machine,
    Ptr<McpttOffNetworkFloorParticipantState> oldState,
    Ptr<McpttOffNetworkFloorParticipantState> newState)
{
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeATxCb(const TestFloorMachine& machine,
                                                   const McpttMsg& msg)
{
    if (msg.GetInstanceTypeId() == McpttFloorMsgGranted::GetTypeId())
    {
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeBRxCb(const TestFloorMachine& machine,
                                                   const McpttMsg& msg)
{
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeBStateChangeCb(
    const TestFloorMachine& machine,
    Ptr<McpttOffNetworkFloorParticipantState> oldState,
    Ptr<McpttOffNetworkFloorParticipantState> newState)
{
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeBTxCb(const TestFloorMachine& machine,
                                                   const McpttMsg& msg)
{
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
    ...
    else
    {
        NS_TEST_ASSERT_MSG_EQ(true,
                              false,
                              "UE B sent unexpected " << msg.GetInstanceTypeId().GetName() << ".");
    }
}

void
McpttFloorReleasePreemptedFloorArbitrator::UeCRxCb(const TestFloorMachine& machine,
                                                   const McpttMsg& msg)
{
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeCStateChangeCb(
    const TestFloorMachine& machine,
    Ptr<McpttOffNetworkFloorParticipantState> oldState,
    Ptr<McpttOffNetworkFloorParticipantState> newState)
{
...
void
McpttFloorReleasePreemptedFloorArbitrator::UeCTxCb(const TestFloorMachine& machine,
                                                   const McpttMsg& msg)
{
    if (msg.GetInstanceTypeId() == McpttFloorMsgRequest::GetTypeId())
    {
