...
IntelHttpBaseTestCase::IntelHttpBaseTestCase(const std::string& name)
    : TestCase(name)
{
}
...
IntelHttpTestCaseEventOrder::RequestMainObject(Ptr<const Packet> /* unused */)
{
    ...
    else if (m_nextSessionMainObjectRequest.IsZero())
    {
        ...
        NS_TEST_ASSERT_MSG_GT(m_nextSessionMainObjectRequest,
                              m_mainObjectRequest,
                              "Second session should start after the first");
        NS_TEST_ASSERT_MSG_GT_OR_EQ(
            m_nextSessionMainObjectRequest,
            m_embeddedObjectReceive,
...
{
    NS_TEST_ASSERT_MSG_EQ(m_mainObjectRequestReceived.IsZero(),
                          true,
                          "Server received request for main object several times");
    m_mainObjectRequestReceived = Simulator::Now();
    NS_TEST_ASSERT_MSG_LT_OR_EQ(m_mainObjectRequest,
                                m_mainObjectRequestReceived,
...
{
    NS_TEST_ASSERT_MSG_EQ(m_mainObjectSent.IsZero(),
                          true,
...
IntelHttpTestCaseEventOrder::ReceiveMainObject(Ptr<const Packet> /* unused */,
                                               const Address& /* unused */)
{
    NS_TEST_ASSERT_MSG_EQ(m_mainObjectReceive.IsZero(),
                          true,
                          "Client received main object several times");
    m_mainObjectReceive = Simulator::Now();
    NS_TEST_ASSERT_MSG_LT_OR_EQ(m_mainObjectSent,
                                m_mainObjectReceive,
...
IntelHttpTestCaseEventOrder::RequestEmbeddedObject(Ptr<const Packet> /* unused */)
{
    NS_TEST_ASSERT_MSG_EQ(
        m_embeddedObjectRequest.IsZero(),
        true,
...
{
    NS_TEST_ASSERT_MSG_EQ(
        m_embeddedObjectRequestReceived.IsZero(),
        true,
        "Server received several request for embedded objects. Make sure the test "
        "is configured correctly");
    m_embeddedObjectRequestReceived = Simulator::Now();
    NS_TEST_ASSERT_MSG_LT_OR_EQ(m_embeddedObjectRequest,
                                m_embeddedObjectRequestReceived,
                                "Client must have sent a request for the embedded object before "
                                "the server could receive it");
}
...
IntelHttpTestCaseEventOrder::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "TxRequestMainObject",
        MakeCallback(&IntelHttpTestCaseEventOrder::RequestMainObject, this));
    client->TraceConnectWithoutContext(
        "TxRequestEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseEventOrder::RequestEmbeddedObject, this));
    client->TraceConnectWithoutContext(
        "RxMainObject",
        MakeCallback(&IntelHttpTestCaseEventOrder::ReceiveMainObject, this));
    client->TraceConnectWithoutContext(
        "RxEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseEventOrder::ReceiveEmbeddedObject, this));
    ...
    server->TraceConnectWithoutContext(
        "TxMainObject",
        MakeCallback(&IntelHttpTestCaseEventOrder::SendMainObject, this));
    server->TraceConnectWithoutContext(
        "TxEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseEventOrder::SendEmbeddedObject, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_mainObjectRequest.IsZero(),
                          false,
    ...
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectRequest.IsZero(),
                          false,
                          "Client never requested the embedded object");
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectRequestReceived.IsZero(),
                          false,
                          "Server never received request for the embedded object");
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectSent.IsZero(),
                          false,
                          "Server never sent the embedded object");
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectReceive.IsZero(),
                          false,
                          "Client never received the embedded object");
    NS_TEST_ASSERT_MSG_EQ(m_nextSessionMainObjectRequest.IsZero(),
                          false,
...
void
IntelHttpTestCaseTxRxCountSizes::RxClient(Ptr<const Packet> /*unused*/, const Address& /* unused */)
{
    m_rxCountClient++;
}
...
void
IntelHttpTestCaseTxRxCountSizes::RxServer(Ptr<const Packet> /*unused*/, const Address& /* unused */)
{
    m_rxCountServer++;
}
...
IntelHttpTestCaseTxRxCountSizes::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "Tx",
        MakeCallback(&IntelHttpTestCaseTxRxCountSizes::TxClient, this));
    client->TraceConnectWithoutContext(
        "TxRequestMainObject",
    ...
    client->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&IntelHttpTestCaseTxRxCountSizes::RxClient, this));
    client->TraceConnectWithoutContext(
        "RxMainObject",
        MakeCallback(&IntelHttpTestCaseTxRxCountSizes::ReceiveMainObject, this));
    client->TraceConnectWithoutContext(
        "RxEmbeddedObject",
    ...
    server->TraceConnectWithoutContext(
        "Tx",
        MakeCallback(&IntelHttpTestCaseTxRxCountSizes::TxServer, this));
    server->TraceConnectWithoutContext(
        "TxMainObject",
        MakeCallback(&IntelHttpTestCaseTxRxCountSizes::SendMainObject, this));
    server->TraceConnectWithoutContext(
        "TxEmbeddedObject",
    ...
    server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&IntelHttpTestCaseTxRxCountSizes::RxServer, this));
    server->TraceConnectWithoutContext(
        "RxRequestMainObject",
    ...
    NS_TEST_ASSERT_MSG_GT(m_mainObjectRequestsReceived,
                          0,
    ...
    NS_TEST_ASSERT_MSG_GT(m_embeddedObjectRequestsSent,
                          0,
...
IntelHttpTestCaseCacheTotal::CacheHit(uint16_t currentObject)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(currentObject,
                          EMBEDDED_OBJECTS - m_countRemaining,
...
IntelHttpTestCaseCacheTotal::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "TxRequestMainObject",
        MakeCallback(&IntelHttpTestCaseCacheTotal::RequestMainObject, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_countHit + m_countMiss,
                          EMBEDDED_OBJECTS,
...
void
IntelHttpTestCaseAllCacheHits::CacheHit(uint16_t /*unused*/)
{
...
void
IntelHttpTestCaseAllCacheHits::CacheMiss(uint16_t /*unused*/)
{
...
IntelHttpTestCaseAllCacheHits::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "CacheHit",
        MakeCallback(&IntelHttpTestCaseAllCacheHits::CacheHit, this));
    client->TraceConnectWithoutContext(
        "CacheMiss",
        MakeCallback(&IntelHttpTestCaseAllCacheHits::CacheMiss, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_countHit,
                          EMBEDDED_OBJECTS,
...
void
IntelHttpTestCaseAllCacheMisses::CacheHit(uint16_t /*unused*/)
{
    // Since there doesn't seem to be a test fail macro,
    // then asserting equality on two unequal things should work...
...
void
IntelHttpTestCaseAllCacheMisses::CacheMiss(uint16_t /*unused*/)
{
    // We'll still verify the count just in case
    m_countMiss++;
}
...
IntelHttpTestCaseAllCacheMisses::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "CacheHit",
        MakeCallback(&IntelHttpTestCaseAllCacheMisses::CacheHit, this));
    client->TraceConnectWithoutContext(
        "CacheMiss",
        MakeCallback(&IntelHttpTestCaseAllCacheMisses::CacheMiss, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_countMiss,
                          EMBEDDED_OBJECTS,
...
IntelHttpTestCaseEmbeddedObjectSequence::RequestEmbeddedObject(Ptr<const Packet> packet)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(header.GetNumberEmbeddedObjects(),
                          m_lastRequestedObjectNumber + 1,
...
IntelHttpTestCaseEmbeddedObjectSequence::TransmitEmbeddedObject(Ptr<const Packet> packet)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(header.GetNumberEmbeddedObjects(),
                          m_lastTransmittedObjectNumber + 1,
                          "Current embedded object number does not match expected number");
...
IntelHttpTestCaseEmbeddedObjectSequence::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_lastRequestedObjectNumber,
                          EMBEDDED_OBJECTS - 1,
                          "Incorrect number of embedded objects requested");
    NS_TEST_ASSERT_MSG_EQ(m_lastTransmittedObjectNumber,
                          EMBEDDED_OBJECTS - 1,
...
IntelHttpTestCaseParseTime::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "TxRequestMainObject",
        MakeCallback(&IntelHttpTestCaseParseTime::RequestMainObject, this));
    client->TraceConnectWithoutContext(
        "RxMainObject",
        MakeCallback(&IntelHttpTestCaseParseTime::ReceiveMainObject, this));
    ...
    client->TraceConnectWithoutContext(
        "RxEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseParseTime::ReceiveEmbeddedObject, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_mainObjectEnd - m_mainObjectStart,
                          Seconds(PARSE_TIME),
                          "Main object parsing took an unexpected amount of time");
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectEnd - m_embeddedObjectStart,
                          Seconds(PARSE_TIME),
...
IntelHttpTestCaseReadTime::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "TxRequestMainObject",
        MakeCallback(&IntelHttpTestCaseReadTime::RequestMainObject, this));
    client->TraceConnectWithoutContext(
        "RxEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseReadTime::ReceiveEmbeddedObject, this));

    // We'll abort before this
    Simulator::Stop(DEFAULT_STOP_TIME);
    Simulator::Run();
    Simulator::Destroy();
    NS_TEST_ASSERT_MSG_EQ(m_readEnd - m_readStart,
                          Seconds(READ_TIME),
                          "Embedded object parsing took an unexpected amount of time");
}
...
IntelHttpTestCaseObjectInterArrival::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_IatEnd - m_IatStart,
...
IntelHttpTestCaseIpv6::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "TxRequestMainObject",
        MakeCallback(&IntelHttpTestCaseIpv6::RequestMainObject, this));
    server->TraceConnectWithoutContext(
        "RxRequestMainObject",
        MakeCallback(&IntelHttpTestCaseIpv6::ReceiveMainObjectRequest, this));
    client->TraceConnectWithoutContext(
        "RxEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseIpv6::ReceiveEmbeddedObject, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_mainObjectRequestReceived,
                          true,
                          "Server never received a request for a Main Object");
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectReceived,
                          true,
...
IntelHttpTestCaseHeaderContent::ReceiveMainObjectRequest(Ptr<const Packet> packet, const Address&)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(header.GetRequestType(),
                          psc::IntelHttpHeader::Type::Main,
                          "Callback mismatch: Server received request for Main object that has "
...
IntelHttpTestCaseHeaderContent::RequestEmbeddedObject(Ptr<const Packet> packet)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(
        header.GetRequestType(),
        psc::IntelHttpHeader::Type::Embedded,
...
IntelHttpTestCaseHeaderContent::DoRun()
{
    ...
    client->TraceConnectWithoutContext(
        "RxMainObject",
        MakeCallback(&IntelHttpTestCaseHeaderContent::ReceiveMainObject, this));
    client->TraceConnectWithoutContext(
        "RxEmbeddedObject",
    ...
    server->TraceConnectWithoutContext(
        "TxMainObject",
        MakeCallback(&IntelHttpTestCaseHeaderContent::SendMainObject, this));
    server->TraceConnectWithoutContext(
        "TxEmbeddedObject",
        MakeCallback(&IntelHttpTestCaseHeaderContent::SendEmbeddedObject, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectRequest,
                          true,
                          "Client never requested an embedded object");
    NS_TEST_ASSERT_MSG_EQ(m_embeddedObjectRequestReceived,
                          true,
                          "Server never received a request for an embedded object");
