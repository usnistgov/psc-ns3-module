...
UavMobilityEnergyModelTestCaseZeroEnergyCost::DoRun()
{
    ...
    Simulator::Schedule(Seconds(1), &UavMobilityEnergyModel::Ascend, model, MOVEMENT_SPEED);
    Simulator::Schedule(Seconds(2), &UavMobilityEnergyModel::Descend, model, MOVEMENT_SPEED);
    Simulator::Schedule(Seconds(3), &UavMobilityEnergyModel::Move, model, MOVE_VECTOR);
...
UavMobilityEnergyModelTestCaseMoveEnergy::DoRun()
{
    ...
    Ptr<UavMobilityEnergyModel> model = CreateObject<UavMobilityEnergyModel>();
    model->SetAttribute("MoveEnergyConversionFactor", DoubleValue(MOVE_CONVERSION_FACTOR));
    model->SetAttribute("AscendEnergyConversionFactor", DoubleValue(ASCEND_CONVERSION_FACTOR));
    model->Init(node, energySource);
    node->AggregateObject(model);

    const double MOVE_X_AMPERAGE = MOVE_CONVERSION_FACTOR * MOVE_X.GetLength();
    const double MOVE_XZ_AMPERAGE =
        MOVE_CONVERSION_FACTOR * MOVE_XZ_2D.GetLength() + ASCEND_CONVERSION_FACTOR * MOVE_XZ.z;
...
UavMobilityEnergyModelTestCaseLowEnergyTrace::DoRun()
{
    ...
    const double MAX_RUN_TIME = std::floor(INITIAL_ENERGY / (VOLTAGE * HOVER_COST));

    // Make sure the test itself makes sense
    NS_TEST_ASSERT_MSG_GT(MAX_RUN_TIME, 1, "Make sure we run for more than one second");
    ...
    energySource->SetAttribute("BasicEnergyLowBatteryThreshold", DoubleValue(.5));

    Ptr<UavMobilityEnergyModel> model = CreateObject<UavMobilityEnergyModel>();
    model->Init(node, energySource);
    node->AggregateObject(model);

    model->TraceConnectWithoutContext(
        "EnergyDepleted",
        MakeCallback(&UavMobilityEnergyModelTestCaseLowEnergyTrace::LowEnergy, this));

    // We should not have a recharged event
    model->TraceConnectWithoutContext(
        "EnergyRecharged",
        MakeCallback(&UavMobilityEnergyModelTestCaseLowEnergyTrace::HighEnergy, this));

    Simulator::Schedule(Seconds(1), &UavMobilityEnergyModel::Hover, model);
    // Be sure we stop before we go over the energy source's capacity
    Simulator::Schedule(Seconds(MAX_RUN_TIME), &UavMobilityEnergyModel::Stop, model);
    Simulator::Stop(Seconds(MAX_RUN_TIME));
    Simulator::Run();

    NS_TEST_ASSERT_MSG_EQ(m_lowEnergyCount, 1, "Low Energy callback should be called once");
    NS_TEST_ASSERT_MSG_EQ(m_highEnergyCount, 0, "Recharge event should not occur");
...
UavMobilityEnergyModelTestCaseLiIonEnergySource::DoRun()
{
    ...
    // Energy Source
    NS_TEST_ASSERT_MSG_LT(liIon->GetRemainingEnergy(),
                          liIon->GetInitialEnergy(),
...
UavMobilityEnergyModelTestCaseMobilityModel::DoRun()
{
    ...
    // Make sure we don't consume more then the possible energy
    const double MAX_RUN_TIME = std::floor(INITIAL_ENERGY / (VOLTAGE * MOVE_COST));

    // Make sure the test itself makes sense
    NS_TEST_ASSERT_MSG_GT(MAX_RUN_TIME, 1, "Test must run for at least one second");
...
UavMobilityEnergyModelTestCaseChangeCostsRuntime::DoRun()
{
    ...
    // Make sure we don't consume more then the possible energy
    const double MAX_RUN_TIME = std::floor(INITIAL_ENERGY / (VOLTAGE * MOVE_COST_INITIAL));

    // Make sure the test itself makes sense
    NS_TEST_ASSERT_MSG_GT(MAX_RUN_TIME, DURATION, "Test must run for at least the duration");
    ...
    model->SetAttribute("AscendEnergyConversionFactor", DoubleValue(MOVE_COST_INITIAL));
    ...
    NS_TEST_ASSERT_MSG_EQ(energySource->GetRemainingEnergy(),
                          INITIAL_ENERGY - (INITIAL_COST_CONSUMPTION + FINAL_COST_CONSUMPTION),
...
UavMobilityEnergyModelTestCaseTotalEnergyConsumption::DoRun()
{
    ...
    // Make sure the test itself makes sense
    NS_TEST_ASSERT_MSG_GT(MAX_RUN_TIME, DURATION, "Test must run for the complete duration");
    ...
    model->TraceConnectWithoutContext(
        ...
        MakeCallback(&UavMobilityEnergyModelTestCaseTotalEnergyConsumption::TraceEnergy, this));
    model->Init(node, energySource);
...
class UavMobilityEnergyModelTestCaseState : public TestCase
{
     ...
     */
    void ValidateState(Ptr<UavMobilityEnergyModel> energyModel,
                       UavMobilityEnergyModel::State expected);

  public:
    UavMobilityEnergyModelTestCaseState();
    void DoRun() override;
    void TraceState(UavMobilityEnergyModel::State oldState,
                    UavMobilityEnergyModel::State newState,
                    Vector velocity);
};
...
UavMobilityEnergyModelTestCaseState::DoRun()
{
    ...
    model->TraceConnectWithoutContext(
        "State",
        MakeCallback(&UavMobilityEnergyModelTestCaseState::TraceState, this));
    model->Init(node, energySource);
...
void
UavMobilityEnergyModelTestCaseState::TraceState(UavMobilityEnergyModel::State oldState,
                                                UavMobilityEnergyModel::State newState,
                                                Vector velocity)
{
    m_state = newState;
...
void
UavMobilityEnergyModelTestCaseState::ValidateState(Ptr<UavMobilityEnergyModel> energyModel,
                                                   UavMobilityEnergyModel::State expected)
{
    NS_TEST_ASSERT_MSG_EQ(energyModel->GetState(),
                          expected,
                          "Model state does not match expected state");

    NS_TEST_ASSERT_MSG_EQ(m_state, expected, "Traced state does not match expected state");
}
...
UavMobilityEnergyModelTestCaseTraceCurrent::DoRun()
{
    ...
    model->TraceConnectWithoutContext(
        "Current",
        MakeCallback(&UavMobilityEnergyModelTestCaseTraceCurrent::TraceCurrent, this));
    model->Init(node, energySource);

    mobilityModel->SetVelocity(STOPPED_VELOCITY);
    NS_TEST_ASSERT_MSG_EQ(m_current, 0, "Traced current is incorrect for stop");

    mobilityModel->SetVelocity(MOVE_X_VELOCITY);
    NS_TEST_ASSERT_MSG_EQ(m_current, MOVE_COST, "Traced current is incorrect for move X");

    mobilityModel->SetVelocity(MOVE_ASCEND_Z_VELOCITY);
    NS_TEST_ASSERT_MSG_EQ(m_current, ASCEND_COST, "Traced current is incorrect for ascend");

    mobilityModel->SetVelocity(MOVE_DESCEND_Z_VELOCITY);
    NS_TEST_ASSERT_MSG_EQ(m_current, DESCEND_COST, "Traced current is incorrect for descend");

    mobilityModel->SetVelocity(MOVE_Y_VELOCITY);
    NS_TEST_ASSERT_MSG_EQ(m_current, MOVE_COST, "Traced current is incorrect for move Y");
    ...
    mobilityModel->SetVelocity(STOPPED_VELOCITY);
    mobilityModel->SetPosition({0, 0, 1});
    NS_TEST_ASSERT_MSG_EQ(m_current, HOVER_COST, "Traced current is incorrect for hover");

    // Stop Hovering
    mobilityModel->SetPosition({0, 0, 0});
    NS_TEST_ASSERT_MSG_EQ(m_current, 0, "Traced current is incorrect for stop after hover");

    Simulator::Destroy();
}

void
UavMobilityEnergyModelTestCaseTraceCurrent::TraceCurrent(double oldCurrent,
                                                         double newCurrent,
                                                         Vector velocity)
{
...
UavMobilityEnergyModelTestCaseAttributeEnergySource::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_NE(model->GetEnergySource(), nullptr, "Energy source should be set");
...
class UavMobilityEnergyModelTestCaseFixedStateChangeSpeed : public TestCase
{
  ...
  public:
    ...
    void DoRun() override;
    void StateTrace(UavMobilityEnergyModel::State oldState,
                    UavMobilityEnergyModel::State newState,
                    Vector velocity);
...
UavMobilityEnergyModelTestCaseFixedStateChangeSpeed::DoRun()
{
    ...
    model->TraceConnectWithoutContext(
        "State",
        MakeCallback(&UavMobilityEnergyModelTestCaseFixedStateChangeSpeed::StateTrace, this));
    model->TraceConnectWithoutContext(
        "Current",
        MakeCallback(&UavMobilityEnergyModelTestCaseFixedStateChangeSpeed::CurrentTrace, this));
    ...
    bool matchesSlowA = TestDoubleIsEqual(model->GetCurrentA(), ASCEND_SLOW_AMPERAGE);
    NS_TEST_ASSERT_MSG_EQ(matchesSlowA, false, "Should not match the slow speeds amperage");

    const double ASCEND_FAST_AMPERAGE = ASCEND_FAST.z * ASCEND_CONVERSION_FACTOR;
    bool matchesFastA = TestDoubleIsEqual(model->GetCurrentA(), ASCEND_FAST_AMPERAGE);
    NS_TEST_ASSERT_MSG_EQ(matchesFastA, true, "Should match the fast speeds amperage");

    Simulator::Destroy();
}

void
UavMobilityEnergyModelTestCaseFixedStateChangeSpeed::StateTrace(
    UavMobilityEnergyModel::State oldState,
    UavMobilityEnergyModel::State newState,
    Vector velocity)
{
    m_stateChangedCount++;
...
void
UavMobilityEnergyModelTestCaseFixedStateChangeSpeed::CurrentTrace(double oldCurrent,
                                                                  double newCurrent,
                                                                  Vector velocity)
{
...
UavMobilityEnergyModelTestCaseVelocityTraces::DoRun()
{
    ...
    model->TraceConnectWithoutContext(
        "State",
        MakeCallback(&UavMobilityEnergyModelTestCaseVelocityTraces::StateTrace, this));
    model->TraceConnectWithoutContext(
        "Current",
        MakeCallback(&UavMobilityEnergyModelTestCaseVelocityTraces::CurrentTrace, this));

    // ----- MOVE -----
    mobilityModel->SetVelocity(MOVE);
    bool isStateVelocityCorrect = CompareVectors(MOVE, m_stateVelocity);
    NS_TEST_ASSERT_MSG_EQ(isStateVelocityCorrect,
                          true,
                          "State velocity should match actual velocity");

    bool isCurrentVelocityCorrect = CompareVectors(MOVE, m_currentVelocity);
    NS_TEST_ASSERT_MSG_EQ(isCurrentVelocityCorrect,
                          true,
...
void
UavMobilityEnergyModelTestCaseVelocityTraces::StateTrace(UavMobilityEnergyModel::State oldState,
                                                         UavMobilityEnergyModel::State newState,
                                                         Vector velocity)
{
...
void
UavMobilityEnergyModelTestCaseVelocityTraces::CurrentTrace(double oldCurrent,
                                                           double newCurrent,
                                                           Vector velocity)
{
