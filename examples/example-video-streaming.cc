 ...
 * be modified to better match our needs using the 'maxUdpPayloadSize' parameter.
 *
 ...
 * - distribution: type of video, which can be '720p-bright' (default), '720p-dark', '1080p-bright',
 * or '1080p-dark'
 ...
 * - maxUdpPayloadSize: maximum UDP payload size [12, 65500] (default: 1472)
 */
...
void
RxPacketTrace(Ptr<OutputStreamWrapper> stream,
              std::string context,
              Ptr<const Packet> p,
...
main(int argc, char* argv[])
{
    ...
    uint16_t maxUdpPayloadSize = 1500;
    ...
    cmd.AddValue(
        "boostPercentile",
        "Percentile from which the size and intervals are pulled from during the initial boost",
        boostPercentile);
    cmd.AddValue("maxUdpPayloadSize",
                 "Maximum size (in Bytes) of the UDP payloads generated by the application",
                 maxUdpPayloadSize);
    cmd.Parse(argc, argv);

    if (distribution != "720p-bright" && distribution != "720p-dark" &&
        distribution != "1080p-bright" && distribution != "1080p-dark")
    {
        std::cerr << "Error, supported distributions are '720p-bright', '720p-dark', "
                     "'1080p-bright', and '1080p-dark', but user requested '"
    ...
    {
        std::cerr
            << "Error, percentile value must be between 0 and 100 (inclusive), but user requested "
            << boostPercentile << "." << std::endl;
        return -1;
    }

    if (maxUdpPayloadSize < 12 || maxUdpPayloadSize > 65500)
    {
        std::cerr << "Error, maximum UDP datagram size must be between 12 and 65500 (inclusive), "
                     "but user requested "
                  << maxUdpPayloadSize << "." << std::endl;
        return -1;
    }
    ...
    streamingServer->SetAttribute("MaxUdpPayloadSize", UintegerValue(maxUdpPayloadSize));
    ...
    *packetOutputStream->GetStream() << "time(sec)\ttx/rx\tPktSize(bytes)" << std::endl;
    ...
    apps.Get(0)->TraceConnect("Tx",
                              oss.str(),
                              MakeBoundCallback(&TxPacketTrace, packetOutputStream));
    oss.str("rx\t");
    apps.Get(1)->TraceConnect("Rx",
                              oss.str(),
                              MakeBoundCallback(&RxPacketTrace, packetOutputStream));
