...
{
    void RxTrace(Ptr<const Application> app,
                 uint16_t callId,
                 Ptr<const Packet> pkt,
                 const TypeId& headerType); ///< Trace sink function
    void TxTrace(Ptr<const Application> app,
                 uint16_t callId,
                 Ptr<const Packet> pkt,
                 const TypeId& headerType);                                 ///< Trace sink function
    void RsrpMeasurementTrace(uint64_t imsi, uint16_t cellId, double rsrp); ///< Trace sink function
    void FloorControlStateTrace(
        uint32_t mcpttUserId,
        uint16_t callId,
        const std::string& selected,
        const std::string& typeIdName,
        const std::string& oldStateName,
        const std::string& newStateName); ///< Trace sink for floor participant state change
    ...
    Ptr<netsimulyzer::StateTransitionSink> m_floorParticipant{
        nullptr};                                             ///< Plot for in network floor control
...
struct ServerCallTracer
{
    void FloorControlStateTrace(
        ...
        const std::string& typeIdName,
        const std::string& oldStateName,
        const std::string& newStateName); //!< Trace sink for floor control server state change

    uint16_t m_callId;                                        //!< The ID of the call.
    Ptr<McpttOnNetworkFloorArbitrator> m_arbitrator{nullptr}; //!< The floor arbitrator.

#ifdef HAS_NETSIMULYZER
    Ptr<netsimulyzer::StateTransitionSink> m_floorArbitrator{
        nullptr}; //!< Plot for on network floor control server
#endif
};
 ...
 */
void
MessagesTracer::RxTrace(Ptr<const Application> app,
                        uint16_t callId,
                        Ptr<const Packet> pkt,
                        const TypeId& headerType)
{
    if (headerType == McpttMediaMsg::GetTypeId())
    {
        ...
        // CallID for the off network will be initially 2 (on network), then a randomly generated
        // one
        if (callId <= 3)
        {
            m_nRxMsgsOn++;
        ...
        if (m_mediaDelaySeries != nullptr)
        {
            ...
            m_mediaDelaySeries->Append(Simulator::Now().GetSeconds(),
                                       (Simulator::Now() - timestamp).GetMilliSeconds());
        }
#endif
    }
 ...
 */
void
MessagesTracer::TxTrace(Ptr<const Application> app,
                        uint16_t callId,
                        Ptr<const Packet> pkt,
                        const TypeId& headerType)
{
...
void
MessagesTracer::FloorControlStateTrace(uint32_t mcpttUserId,
                                       uint16_t callId,
                                       const std::string& /* selected */,
                                       const std::string& typeIdName,
                                       const std::string& oldStateName,
                                       const std::string& newStateName)
{
    ...
    if (m_floorParticipant != nullptr)
    {
        ...
        if (m_accessTimeSeries != nullptr)
        {
            ...
            //*applicationLog << sstream.str () << " s : CallId " << callId << " UserId " <<
            // mcpttUserId << " state: " << newStateName << "\n"; calculate access time
            ...
            if (m_accessTimeInfo.second == "'U: pending Request'")
            {
                ...
                {
                    m_accessTimeSeries->Append(
                        Simulator::Now().GetSeconds(),
                        (Simulator::Now() - m_accessTimeInfo.first).GetMilliSeconds());
                    return;
                }
            }
...
void
ServerCallTracer::FloorControlStateTrace(uint32_t mcpttUserId,
                                         uint16_t callId,
                                         const std::string& /* selected */,
                                         const std::string& typeIdName,
                                         const std::string& oldStateName,
 ...
 */
void
RemoteRsrpMeasurementTrace(Ptr<OutputStreamWrapper> stream,
                           ...
                           double rsrp)
{
    ...
    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << imsi << "\t"
                         << cellId << "\t" << rsrp << "\t" << threshold << "\t" << std::endl;
    ...
    else
    {
        ...
        if (rsrp < threshold &&
            !slUeRrc->IsMonitoringRelayServiceCode(LteSlDiscHeader::DISC_RELAY_ANNOUNCEMENT,
                                                   RELAY_SERVICE_CODE))
        {
            NS_LOG_INFO("Starting Relay Service as Remote UE for UE Node ID: "
                        << ueDevice->GetNode()->GetId() << " IMSI: " << imsi);
#ifdef HAS_NETSIMULYZER
            if (mobilityLog != nullptr)
            {
                ...
                sstream.precision(3);
                sstream << Simulator::Now().GetSeconds();
                *mobilityLog << sstream.str()
                             << " s: Starting Relay Service as Remote UE for UE Node ID: "
                             << ueDevice->GetNode()->GetId() << "\n";
            }
#endif
 ...
 */
void
Pc5ConnectionStatusTrace(Ptr<McpttServerApp> serverApp,
                         uint16_t callId,
                         Ptr<Node> node,
                         uint32_t srcL2Id,
                         uint32_t dstL2Id,
                         ...
                         uint32_t iface)
{
    if (status == LteSlBasicUeController::SECURE_ESTABLISHED &&
        role == LteSlUeRrc::RelayRole::RemoteUE)
    {
        ...
        NS_LOG_DEBUG("Call " << callId << " user "
                             << node->GetApplication(0)->GetObject<McpttPttApp>()->GetUserId()
                             << " address " << ipv6Addr);
        if (call != nullptr)
        {
            Ptr<McpttOnNetworkFloorArbitrator> arbitrator = call->GetArbitrator();
            Ptr<McpttOnNetworkFloorTowardsParticipant> participantFloorCtrl =
                arbitrator->GetParticipantByUserId(
                    node->GetApplication(0)->GetObject<McpttPttApp>()->GetUserId());
            // Address currentAddr = participantFloorCtrl->GetPeerAddress ();
            // Inet6SocketAddress socketAddr = Inet6SocketAddress::ConvertFrom
            // (participantFloorCtrl->GetPeerAddress ()); socketAddr.SetIpv6 (ipv6Addr);
...
main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::psc::McpttOnNetworkFloorArbitrator::QueueingSupported",
                       BooleanValue(queueing));
    if (queueing)
    {
    ...
    {
        std::cout << "Cannot enable visualization traces because netsimulyzer module not enabled"
                  << std::endl;
        return -1;
    }
#endif
     ...
     * TEAM 1:
     *********
     *                                         team1StartMoving          enteringBuildingTime
     *stopMoving V                         V                  V
     *  --------|--------------------------|-----------|-------------------------|-----------------|~|------------|------------->
     *          ^                          ^ ^             t team1StartTime inNetworkAppStartTime
     *inNetworkAppStopTime
     *          |--------team1startDelayMovement-------|
     *          |--startDelayInNetworkApp--|-----------------
     *durationTimeInNetworkApp----------------------------|
     ...
     * TEAM 2:
     *********
     *                                         team2StartMoving          enteringBuildingTime
     *stopMoving V                         V                  V
     *  --------|--------------------------|-----------|-------------------------|-----------------|~|------------|------------->
     *          ^                          ^                                     ^ ^             t
     *  team2StartTime           inNetworkAppStartTime                 inNetworkAppStopTime ^
     *                                                                outOfNetworkAppStartTime
     *outOfNetworkAppStopTime
     ...
     * TEAM 3:
     *********
     *                                         team3StartMoving          enteringBuildingTime
     *stopMoving V                         V                  V
     *  --------|--------------------------|-----------|----------------|--------|--------|~|------|~|------------|------------->
     *          ^                          ^                            ^                  ^ ^ t
     *  team3StartTime         inNetworkAppStartTime                 RelayUE           RemoteUEs
     *inNetworkAppStopTime StartsRelayService  StartRelayService
    ...
    double team2durationTimeInNetworkApp =
        delayToReachBuilding + (team2startDelayMovement - startDelayInNetworkApp);     // s
    ...
    double startDelayRelayServiceRelayUE =
        team3startDelayMovement + delayToReachBuilding - 1.0; // s

    // Calculate simTime depending on last team's start time and corresponding timing
    double simTime = team3StartTime + team3startDelayMovement + delayToReachBuilding +
                     durationMovementInsideBuilding + 1.0; // s
    ...
    Config::SetDefault("ns3::psc::McpttStateMachineStats::OutputFileName",
                       StringValue("mcptt-operational-modes-mobility-state-machine-stats.dat"));
    Config::SetDefault("ns3::psc::McpttMsgStats::OutputFileName",
                       StringValue("mcptt-operational-modes-mobility-msg-stats.dat"));
    ...
    // TODO: We need to deactivate RLF. Otherwise we run into problems with RNTI mismatch for PC5
    // signaling for Remote UEs
    Config::SetDefault("ns3::LteUePhy::EnableRlfDetection", BooleanValue(false));

    // Configure the UE for UE_SELECTED scenario
    Config::SetDefault("ns3::LteUeMac::SlGrantMcs", UintegerValue(16));
    Config::SetDefault("ns3::LteUeMac::SlGrantSize",
                       UintegerValue(6)); // The number of RBs allocated per UE for Sidelink
    Config::SetDefault("ns3::LteUeMac::Ktrp", UintegerValue(1));
    Config::SetDefault("ns3::LteUeMac::UseSetTrp", BooleanValue(false));
    Config::SetDefault(
        "ns3::LteUeMac::SlScheduler",
        StringValue("MaxCoverage")); // Values include Fixed, Random, MinPrb, MaxCoverage
    ...
    Config::SetDefault("ns3::NoBackhaulEpcHelper::S5LinkDataRate",
    ...
    // Configure Relay UE (re)selection algorithm
    // (RandomNoReselection|MaxSDRsrpNoReselection|MaxSDRsrp)
    Config::SetDefault("ns3::LteSlBasicUeController::RelayUeSelectionAlgorithm",
                       StringValue("MaxSDRsrpNoReselection"));
    ...
    // ("ns3::Hybrid3gppPropagationLossModel"));
    ...
    for (uint32_t ueNodeIdx = 0; ueNodeIdx < allUeNodes.GetN(); ueNodeIdx++)
    {
        ...
        if (ueNodeIdx % nUEsPerTeam == 0)
        {
            ...
            NS_LOG_INFO("Static UE Node id = [" << allUeNodes.Get(ueNodeIdx)->GetId()
                                                ...
                                                << "]"
                                                   " position: ("
                                                << ue_pos_x_static << " , " << ue_pos_y_static
                                                << ", " << 1.5 << ")");
    ...
    NS_LOG_DEBUG("Building bounding box (" << xMin << ", " << (xMin + 40)
                                           << ", -120, 0, 0.0, 20.0)");
    ...
    Ptr<OutputStreamWrapper> mobilityOutputStream =
        ascii.CreateFileStream("mcptt-operational-modes-mobility-course-change.dat");
    // eNodeB
    ...
    else
    {
        ...
        for (uint32_t movingUeNodeIdx = 0; movingUeNodeIdx < movingUEsNodes.GetN();
             movingUeNodeIdx++)
        {
            ...
            double movementStopTime =
                movementStartTime + delayToReachBuilding + durationMovementInsideBuilding;
            ...
            Waypoint wpt1(Seconds(movementStartTime + delayToReachBuilding),
                          Vector(door_pos_x, door_pos_y, 1.5));
            ...
            NS_LOG_INFO("Moving UE Node id = [" << allUeNodes.Get(movingUeNodeIdx)->GetId()
                                                ...
                                                << movementStartTime
                                                << " s"
                                                   " reaches the building at "
                                                << movementStartTime + delayToReachBuilding
                                                << " s"
                                                   " on position ("
                                                << door_pos_x << "m, " << door_pos_y
                                                << "m, 1.5)"
                                                   " and stops moving at "
                                                << movementStopTime
                                                << " s"
                                                   " on position ("
                                                << inside_pos_x << "m, " << inside_pos_y
                                                << "m, 1.5)");
        }
    ...
    NetDeviceContainer allUeDevs =
        NetDeviceContainer(NetDeviceContainer(team1UeDevs, team2UeDevs), team3UeDevs);
    ...
    preconfigurationTeam2.preconfigGeneral.carrierFreq =
        enbDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetUlEarfcn();
    preconfigurationTeam2.preconfigGeneral.slBandwidth =
    ...
    for (uint32_t ueNodeIdx = 0; ueNodeIdx < allUeNodes.GetN(); ueNodeIdx++)
    {
        ...
        tracer.m_team = team;
        tracer.m_ueImsi =
            allUeNodes.Get(ueNodeIdx)->GetDevice(0)->GetObject<LteUeNetDevice>()->GetImsi();
    ...
    mcpttClientHelper.SetPusher("ns3::psc::McpttPusher", "Automatic", BooleanValue(true));
    ...
    DynamicCast<McpttPttApp, Application>(team1Apps.Get(1))
        ->GetPusher()
        ->SetAttribute("Automatic", BooleanValue(true));
    DynamicCast<McpttPttApp, Application>(team2Apps.Get(1))
        ->GetPusher()
        ->SetAttribute("Automatic", BooleanValue(true));
    DynamicCast<McpttPttApp, Application>(team3Apps.Get(1))
        ->GetPusher()
        ->SetAttribute("Automatic", BooleanValue(true));
    ...
    for (uint32_t ueNodeIdx = 0; ueNodeIdx < allUeNodes.GetN(); ueNodeIdx++)
    {
        ...
        NS_LOG_INFO("Team = " << team << " UE node id = [" << allUeNodes.Get(ueNodeIdx)->GetId()
                              << "]"
                                 " UE Idx = ["
                              << ueNodeIdx
                              << "]"
        ...
        pttApp->TraceConnectWithoutContext("RxTrace",
                                           MakeCallback(&MessagesTracer::RxTrace, &it->second));
        pttApp->TraceConnectWithoutContext("TxTrace",
                                           MakeCallback(&MessagesTracer::TxTrace, &it->second));
    }
    ...
    double callStopTime =
        team1StartTime + team1startDelayMovement + durationTimeInNetworkAppWhileMoving;
    ...
    NS_LOG_INFO("Team 1 "
                "starts on-network application at "
                << callStartTime
                << " s and "
                   "stops it at "
                << callStopTime << " s");
    ...
    {
        team2UeNodes.Get(ueNodeIdx)->GetObject<Ipv6L3Protocol>()->AddMulticastAddress(
            groupAddress6);
    }
    ...
    callHelper.AddCallOffNetwork(team2Apps,
                                 ...
                                 Seconds(callStopTime));
    NS_LOG_INFO("Team 2 "
                "starts off-network application at "
                << callStartTime
                << " s and "
                   "stops it at "
                << callStopTime << " s");
    ...
    NodeContainer remoteUeNodes =
        NodeContainer(team3UeNodes.Get(1), team3UeNodes.Get(2), team3UeNodes.Get(3));

    NetDeviceContainer relayUeDev = NetDeviceContainer(team3UeDevs.Get(0));
    NetDeviceContainer remoteUeDevs =
        NetDeviceContainer(NetDeviceContainer(team3UeDevs.Get(1), team3UeDevs.Get(2)),
                           team3UeDevs.Get(3));
    ...
    Simulator::Schedule(Seconds(team3StartTime + startDelayRelayServiceRelayUE),
                        &LteSidelinkHelper::StartRelayService,
                        proseHelper,
                        relayUeDev.Get(0),
                        RELAY_SERVICE_CODE,
                        LteSlUeRrc::ModelA,
    ...
    NS_LOG_INFO("Relay UE node id = [" << relayUeNode.Get(0)->GetId()
                                       << "]"
                                          " provides Service Code "
                                       << RELAY_SERVICE_CODE << " and start service at "
                                       << team3StartTime + startDelayRelayServiceRelayUE << " s");
    ...
    Ptr<OutputStreamWrapper> PC5SignalingPacketTraceStream =
        ascii.CreateFileStream("mcptt-operational-modes-mobility-pc5-signaling.dat");
    *PC5SignalingPacketTraceStream->GetStream() << "time(s)\ttxId\tRxId\tmsgType" << std::endl;
    ...
    // relay service as Remote UE when it falls below 'threshold' (See 'RsrpMeasurementTrace'
    // function)
    Ptr<OutputStreamWrapper> RsrpMeasurementsStream =
        ascii.CreateFileStream("mcptt-operational-modes-mobility-rsrp-measurement.dat");
    *RsrpMeasurementsStream->GetStream() << "time(s)\timsi\tcellId\trsrp\tthreshold" << std::endl;
    ...
    for (uint32_t ueIdx = 0; ueIdx < remoteUeNodes.GetN(); ueIdx++)
    {
        ...
        PointerValue ptrOne;
        rrc->TraceConnect("RsrpMeasurement",
                          ctx.str(),
    ...
    if (enableVisualization)
    {
        ...
        orchestrator =
            CreateObject<netsimulyzer::Orchestrator>("mcptt-operational-modes-mobility.json");
        ...
        auto team1Area = CreateObject<netsimulyzer::RectangularArea>(orchestrator,
                                                                     Rectangle{395, 403, -25, -15});
        team1Area->SetAttribute("Fill", EnumValue(netsimulyzer::RectangularArea::DrawMode::Hidden));
        team1Area->SetAttribute("Border",
                                EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
        // Red
        team1Area->SetAttribute("BorderColor", teamOneColor);

        auto team2Area = CreateObject<netsimulyzer::RectangularArea>(orchestrator,
                                                                     Rectangle{395, 403, -65, -55});
        team2Area->SetAttribute("Fill", EnumValue(netsimulyzer::RectangularArea::DrawMode::Hidden));
        team2Area->SetAttribute("Border",
                                EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
        // Green
        team2Area->SetAttribute("BorderColor", teamTwoColor);

        auto team3Area =
            CreateObject<netsimulyzer::RectangularArea>(orchestrator,
                                                        Rectangle{395, 403, -105, -95});
        team3Area->SetAttribute("Fill", EnumValue(netsimulyzer::RectangularArea::DrawMode::Hidden));
        team3Area->SetAttribute("Border",
                                EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
        // Blue
        ...
        //  {
        //  Ptr<netsimulyzer::Decoration> antenna = CreateObject<netsimulyzer::Decoration>
        //  (orchestrator); antenna->SetAttribute ("Model", StringValue
        //  ("non-distributable/models/props/Cylinder.obj")); antenna->SetAttribute ("Height",
        //  OptionalValue<double> (3)); antenna->SetAttribute ("Position", Vector3DValue
        //  (Vector3D(1, -59, 28))); eNodebAntennas.push_back (antenna); antenna =
        //  CreateObject<netsimulyzer::Decoration> (orchestrator); antenna->SetAttribute ("Model",
        //  StringValue ("non-distributable/models/props/Cylinder.obj")); antenna->SetAttribute
        //  ("Height", OptionalValue<double> (3)); antenna->SetAttribute ("Position", Vector3DValue
        //  (Vector3D(-1, -61, 28))); eNodebAntennas.push_back (antenna); antenna =
        //  CreateObject<netsimulyzer::Decoration> (orchestrator); antenna->SetAttribute ("Model",
        //  StringValue ("non-distributable/models/props/Cylinder.obj")); antenna->SetAttribute
        //  ("Height", OptionalValue<double> (3)); antenna->SetAttribute ("Position", Vector3DValue
        //  (Vector3D(-1, -61, 28))); eNodebAntennas.push_back (antenna);
        //  }
        ...
        Simulator::Schedule(Seconds(15),
                            &netsimulyzer::LogStream::Write,
                            applicationLog,
                            "15 s: Team 1 starts group call\n");
        Simulator::Schedule(Seconds(15),
                            &netsimulyzer::LogStream::Write,
                            applicationLog,
                            "15 s: Team 2 starts group call\n");
        Simulator::Schedule(Seconds(15),
                            &netsimulyzer::LogStream::Write,
                            applicationLog,
                            "15 s: Team 3 starts group call\n");
        Simulator::Schedule(Seconds(75),
                            &netsimulyzer::LogStream::Write,
                            applicationLog,
                            "75 s: Team 1 stops group call\n");
        Simulator::Schedule(Seconds(120),
                            &netsimulyzer::LogStream::Write,
                            applicationLog,
                            "120 s: Team 2 stops group call\n");
        Simulator::Schedule(Seconds(165),
                            &netsimulyzer::LogStream::Write,
                            applicationLog,
                            "165 s: Team 3 stops group call\n");
        ...
        Simulator::Schedule(Seconds(30),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "30 s: Team 1 starts moving\n");
        Simulator::Schedule(Seconds(45),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "45 s: Team 1 enters building\n");
        Simulator::Schedule(Seconds(75),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "75 s: Team 1 stops moving\n");
        Simulator::Schedule(Seconds(75),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "75 s: Team 2 starts moving\n");
        Simulator::Schedule(Seconds(90),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "90 s: Team 2 enters building\n");
        Simulator::Schedule(Seconds(120),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "120 s: Team 2 stops moving\n");
        Simulator::Schedule(Seconds(120),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "135 s: Team 3 starts moving\n");
        Simulator::Schedule(Seconds(135),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "120 s: Team 3 enters building\n");
        Simulator::Schedule(Seconds(165),
                            &netsimulyzer::LogStream::Write,
                            mobilityLog,
                            "165 s: Team 3 enters building\n");
        ...
        for (groupIdToOnNetworkCallIdIt = groupIdToOnNetworkCallId.begin();
             groupIdToOnNetworkCallIdIt != groupIdToOnNetworkCallId.end();
             groupIdToOnNetworkCallIdIt++)
        {
            Ptr<McpttServerCall> serverCall =
                serverAppCalls.Get(groupIdToOnNetworkCallIdIt->second)
                    ->GetObject<McpttServerCall>();
            ServerCallTracer serverCallTracer;
            std::map<uint16_t, ServerCallTracer>::iterator serverIt;
            serverCallTracers.insert(
                std::pair<uint16_t, ServerCallTracer>(groupIdToOnNetworkCallIdIt->first,
                                                      serverCallTracer));
            serverIt = serverCallTracers.find(groupIdToOnNetworkCallIdIt->first);
            CategoryValuePair floorArbitratorStateValue;
            floorArbitratorStateValue.key =
                McpttOnNetworkFloorArbitratorStateStartStop::GetStateId().GetNumber();
            floorArbitratorStateValue.value =
                McpttOnNetworkFloorArbitratorStateStartStop::GetStateId().GetName();
            floorArbitratorStates.push_back(floorArbitratorStateValue);
            floorArbitratorStateValue.key =
                McpttOnNetworkFloorArbitratorStateIdle::GetStateId().GetNumber();
            floorArbitratorStateValue.value =
                McpttOnNetworkFloorArbitratorStateIdle::GetStateId().GetName();
            floorArbitratorStates.push_back(floorArbitratorStateValue);
            floorArbitratorStateValue.key =
                McpttOnNetworkFloorArbitratorStateTaken::GetStateId().GetNumber();
            floorArbitratorStateValue.value =
                McpttOnNetworkFloorArbitratorStateTaken::GetStateId().GetName();
            floorArbitratorStates.push_back(floorArbitratorStateValue);
            floorArbitratorStateValue.key =
                McpttOnNetworkFloorArbitratorStateRevoke::GetStateId().GetNumber();
            floorArbitratorStateValue.value =
                McpttOnNetworkFloorArbitratorStateRevoke::GetStateId().GetName();
            floorArbitratorStates.push_back(floorArbitratorStateValue);
            floorArbitratorStateValue.key =
                McpttOnNetworkFloorArbitratorStateReleasing::GetStateId().GetNumber();
            floorArbitratorStateValue.value =
                McpttOnNetworkFloorArbitratorStateReleasing::GetStateId().GetName();
            floorArbitratorStates.push_back(floorArbitratorStateValue);
            floorArbitratorStateValue.key =
                McpttOnNetworkFloorArbitratorStateInitialising::GetStateId().GetNumber();
            floorArbitratorStateValue.value =
                McpttOnNetworkFloorArbitratorStateInitialising::GetStateId().GetName();
            floorArbitratorStates.push_back(floorArbitratorStateValue);

            PointerValue floorArbitrator;
            serverCall->GetAttribute("FloorArbitrator", floorArbitrator);
            Ptr<McpttOnNetworkFloorArbitrator> arbitrator =
                floorArbitrator.Get<McpttOnNetworkFloorArbitrator>();
            arbitrator->TraceConnectWithoutContext(
                "StateChangeTrace",
                MakeCallback(&ServerCallTracer::FloorControlStateTrace, &serverIt->second));

            serverIt->second.m_floorArbitrator =
                CreateObject<netsimulyzer::StateTransitionSink>(orchestrator,
                                                                floorArbitratorStates,
                                                                0);
            serverIt->second.m_floorArbitrator->SetAttribute(
                "Name",
                StringValue("Team " + std::to_string(groupIdToOnNetworkCallIdIt->first) +
                            " Floor Arbitrator"));
        ...
        for (rxMessagesPerUeTracerMapIt = rxMessagesPerUeTracerMap.begin();
             rxMessagesPerUeTracerMapIt != rxMessagesPerUeTracerMap.end();
             rxMessagesPerUeTracerMapIt++)
        {
            // Calculate team to which the UE belongs
            ...
            rrc->TraceConnectWithoutContext("RsrpMeasurement",
                                            MakeCallback(&MessagesTracer::RsrpMeasurementTrace,
                                                         &rxMessagesPerUeTracerMapIt->second));

            if (ueNodeIdx == 2 || ueNodeIdx == 6 || ueNodeIdx == 11)
            {
                rxMessagesPerUeTracerMapIt->second.m_rsrpSeries =
                    CreateObject<netsimulyzer::XYSeries>(orchestrator);

                rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->SetAttribute(
                    "Name",
                    StringValue("UE " + std::to_string(ueNodeIdx) + " RSRP"));
                rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->SetAttribute(
                    "LabelMode",
                    StringValue("Hidden"));
                rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->SetAttribute("Connection",
                                                                              StringValue("None"));
                switch (team)
                {
                case 1:
                    rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->SetAttribute("Color",
                                                                                  teamOneColor);
                    break;
                case 2:
                    rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->SetAttribute("Color",
                ...
                rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->GetXAxis()->SetAttribute(
                    "Name",
                    StringValue("Time (s)"));
                ...
                yAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name",
                                                                   StringValue("RSRP (dBm)"));
                yAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("BoundMode",
                                                                   StringValue("Fixed"));
                ...
                // Media delay
                rxMessagesPerUeTracerMapIt->second.m_mediaDelaySeries =
                    CreateObject<netsimulyzer::XYSeries>(orchestrator);

                rxMessagesPerUeTracerMapIt->second.m_mediaDelaySeries->SetAttribute(
                    "Name",
                    StringValue("UE " + std::to_string(ueNodeIdx) + " M2E latency"));
                rxMessagesPerUeTracerMapIt->second.m_mediaDelaySeries->SetAttribute(
                    "LabelMode",
                    StringValue("Hidden"));
                rxMessagesPerUeTracerMapIt->second.m_mediaDelaySeries->SetAttribute(
                ...
                rxMessagesPerUeTracerMapIt->second.m_mediaDelaySeries->GetXAxis()->SetAttribute(
                    "Name",
                    StringValue("Time (s)"));
                rxMessagesPerUeTracerMapIt->second.m_mediaDelaySeries->GetYAxis()->SetAttribute(
                    "Name",
                    StringValue("Latency (ms)"));
            }
            ...
            for (ueCallsIt = ueCalls.begin(); ueCallsIt != ueCalls.end(); ueCallsIt++)
            {
                ...
                if (ueFloorParticipant->GetInstanceTypeId() ==
                    McpttOnNetworkFloorParticipant::GetTypeId())
                {
                    hasOnNetworkCall = true;
                    ueFloorParticipant->GetObject<McpttOnNetworkFloorParticipant>()
                        ->TraceConnectWithoutContext(
                            "StateChangeTrace",
                            MakeCallback(&MessagesTracer::FloorControlStateTrace,
                                         &rxMessagesPerUeTracerMapIt->second));
                }
                else if (ueFloorParticipant->GetInstanceTypeId() ==
                         McpttOffNetworkFloorParticipant::GetTypeId())
                {
                    hasOffNetworkCall = true;
                    ueFloorParticipant->GetObject<McpttOffNetworkFloorParticipant>()
                        ->TraceConnectWithoutContext(
                            "StateChangeTrace",
                            MakeCallback(&MessagesTracer::FloorControlStateTrace,
                                         &rxMessagesPerUeTracerMapIt->second));
            ...
            pusherStates.push_back(pusherStateValue);
            pttApp->GetPusher()->TraceConnectWithoutContext(
                "PushingState",
                MakeCallback(&MessagesTracer::PusherStateTrace,
                             &rxMessagesPerUeTracerMapIt->second));

            // Add pusher state only for Tx nodes
            if (ueNodeIdx == 1 || ueNodeIdx == 5 || ueNodeIdx == 9)
            {
                rxMessagesPerUeTracerMapIt->second.m_pusher =
                    CreateObject<netsimulyzer::StateTransitionSink>(orchestrator, pusherStates, 0);
                rxMessagesPerUeTracerMapIt->second.m_pusher->SetAttribute(
                    "Name",
                    StringValue("UE " + std::to_string(ueNodeIdx) + " Pusher state"));
                rxMessagesPerUeTracerMapIt->second.m_pusher->SetAttribute("LoggingMode",
                                                                          StringValue("None"));
            }
            ...
            floorCtrlStateValue.value =
                McpttOnNetworkFloorParticipantStateStartStop::GetStateId().GetName();
            floorCtrlStates.push_back(floorCtrlStateValue);
            rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                std::pair<std::string, int>(floorCtrlStateValue.value, floorCtrlStateValue.key));

            if (hasOnNetworkCall)
            {
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOnNetworkFloorParticipantStateHasNoPermission::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOnNetworkFloorParticipantStatePendingRelease::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOnNetworkFloorParticipantStatePendingRequest::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOnNetworkFloorParticipantStateHasPermission::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOnNetworkFloorParticipantStateReleasing::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOnNetworkFloorParticipantStateQueued::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
            ...
            if (hasOffNetworkCall)
            {
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOffNetworkFloorParticipantStateSilence::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOffNetworkFloorParticipantStateNoPerm::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOffNetworkFloorParticipantStatePendGrant::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOffNetworkFloorParticipantStatePendReq::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOffNetworkFloorParticipantStateHasPerm::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
                floorCtrlStateValue.key++;
                floorCtrlStateValue.value =
                    McpttOffNetworkFloorParticipantStateQueued::GetStateId().GetName();
                floorCtrlStates.push_back(floorCtrlStateValue);
                rxMessagesPerUeTracerMapIt->second.m_stateToId.insert(
                    std::pair<std::string, int>(floorCtrlStateValue.value,
                                                floorCtrlStateValue.key));
            ...
            {
                rxMessagesPerUeTracerMapIt->second.m_floorParticipant =
                    CreateObject<netsimulyzer::StateTransitionSink>(orchestrator,
                                                                    floorCtrlStates,
                                                                    0);
                rxMessagesPerUeTracerMapIt->second.m_floorParticipant->SetAttribute(
                    "Name",
                    StringValue("UE " + std::to_string(ueNodeIdx) + " Floor control"));
                rxMessagesPerUeTracerMapIt->second.m_floorParticipant->SetAttribute(
                    "LoggingMode",
                    StringValue("None"));
            }
            ...
            if (ueNodeIdx == 1 || ueNodeIdx == 5 || ueNodeIdx == 9)
            {
                rxMessagesPerUeTracerMapIt->second.m_accessTimeSeries =
                    CreateObject<netsimulyzer::XYSeries>(orchestrator);
                rxMessagesPerUeTracerMapIt->second.m_accessTimeSeries->SetAttribute(
                    "Name",
                    StringValue("UE " + std::to_string(ueNodeIdx) + " Access time"));
                rxMessagesPerUeTracerMapIt->second.m_accessTimeSeries->SetAttribute(
                    "LabelMode",
                    StringValue("Hidden"));
                rxMessagesPerUeTracerMapIt->second.m_accessTimeSeries->SetAttribute(
                ...
                rxMessagesPerUeTracerMapIt->second.m_accessTimeSeries->GetXAxis()->SetAttribute(
                    "Name",
                    StringValue("Time (s)"));
                rxMessagesPerUeTracerMapIt->second.m_accessTimeSeries->GetYAxis()->SetAttribute(
                    "Name",
                    StringValue("Access time (ms)"));
            }
        }
             ...
             rxMessagesPerUeTracerMapIt++)
        {
            ...
            if (ueNodeIdx == 2 || ueNodeIdx == 6 || ueNodeIdx == 11)
            {
                // rxMessagesPerUeTracerMapIt->second.m_rsrpSeries->SetAttribute ("Name",
                // StringValue("UE " + std::to_string(ueNodeIdx) + " (team " + std::to_string(team)
                // + ")"));
    ...
    {
        std::cout << "Team: " << it->second.m_team << " UE idx: " << it->second.m_ueIdx
                  << " UE Node ID: " << it->second.m_ueNodeId << " UE IMSI: " << it->second.m_ueImsi
                  ...
                  << " TX packets (off-network) : " << it->second.m_nTxMsgsOff << std::endl;
