...
#define NS_LOG_APPEND_CONTEXT                                                                      \
    if (m_owner)                                                                                   \
    {                                                                                              \
        std::clog << " [callId " << m_owner->GetCallId() << "] ";                                  \
    }
...
{
    static TypeId tid =
        TypeId("ns3::psc::McpttOnNetworkFloorArbitrator")
            .SetParent<Object>()
            .AddConstructor<McpttOnNetworkFloorArbitrator>()
            .AddAttribute("AckRequired",
                          "The flag that indicates if acknowledgement is required.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&McpttOnNetworkFloorArbitrator::m_ackRequired),
                          MakeBooleanChecker())
            .AddAttribute("AudioCutIn",
                          "The flag that indicates if the group is configured for audio cut-in.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&McpttOnNetworkFloorArbitrator::m_audioCutIn),
                          MakeBooleanChecker())
            .AddAttribute("C7",
                          "The initial limit of counter C7 (Floor Idle).",
                          UintegerValue(10),
                          MakeUintegerAccessor(&McpttOnNetworkFloorArbitrator::SetLimitC7),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("C20",
                          "The initial limit of counter C20 (Floor Granted).",
                          UintegerValue(3),
                          MakeUintegerAccessor(&McpttOnNetworkFloorArbitrator::SetLimitC20),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("DualFloorSupported",
                          "The flag that indicates if dual floor control is supported.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&McpttOnNetworkFloorArbitrator::m_dualFloorSupported),
                          MakeBooleanChecker())
            .AddAttribute("DualFloorControl",
                          "The dual floor control state machine.",
            ...
            .AddAttribute("FloorParticipants",
                          "The list of floor participants associated with this arbitrator.",
                          ObjectVectorValue(),
            ...
            .AddAttribute("QueueingSupported",
                          "Whether queueing of floor requests is supported.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&McpttOnNetworkFloorArbitrator::m_queueingSupported),
                          MakeBooleanChecker())
            .AddAttribute("TxSsrc",
                          "The SSRC to use when transmitting a message.",
                          UintegerValue(0),
                          MakeUintegerAccessor(&McpttOnNetworkFloorArbitrator::m_txSsrc),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("T1",
                          "The delay to use for end-of-media timer T1 (Time value)",
                          TimeValue(Seconds(4)),
                          MakeTimeAccessor(&McpttOnNetworkFloorArbitrator::SetDelayT1),
                          MakeTimeChecker())
            .AddAttribute("T2",
                          "The delay to use for stop talking timer T2 (Time value)",
                          TimeValue(Seconds(30)),
                          MakeTimeAccessor(&McpttOnNetworkFloorArbitrator::SetDelayT2),
                          MakeTimeChecker())
            .AddAttribute("T3",
                          "The delay to use for stop talking grace timer T3 (Time value)",
                          TimeValue(Seconds(3)),
                          MakeTimeAccessor(&McpttOnNetworkFloorArbitrator::SetDelayT3),
                          MakeTimeChecker())
            .AddAttribute("T4",
                          "The delay to use for inactivity timer T4 (Time value)",
                          TimeValue(Seconds(30)),
                          MakeTimeAccessor(&McpttOnNetworkFloorArbitrator::SetDelayT4),
                          MakeTimeChecker())
            .AddAttribute("T7",
                          "The delay to use for Floor Idle timer T7 (Time value)",
                          TimeValue(Seconds(1)),
                          MakeTimeAccessor(&McpttOnNetworkFloorArbitrator::SetDelayT7),
                          MakeTimeChecker())
            .AddAttribute("T20",
                          "The delay to use for Floor Granted timer T20 (Time value)",
                          TimeValue(Seconds(1)),
...
void
McpttOnNetworkFloorArbitrator::AddParticipant(
...
void
McpttOnNetworkFloorArbitrator::CallInitialized(
    Ptr<McpttOnNetworkFloorTowardsParticipant> participant,
    bool implicitRequest)
{
    ...
    else
    {
        ...
        NS_LOG_LOGIC("Attempt to initialize call but call already in "
                     << m_state->GetInstanceStateId().GetName());
...
McpttOnNetworkFloorArbitrator::ChangeState(Ptr<McpttOnNetworkFloorArbitratorState> state)
{
    ...
    if (currStateId != stateId)
    {
        ...
        m_stateChangeTrace(GetOwner()->GetCallMachine()->GetUserId(),
                           GetOwner()->GetCallId(),
                           selected,
...
McpttOnNetworkFloorArbitrator::NextSeqNum()
{
    ...
    NS_LOG_LOGIC(" McpttOnNetworkFloorArbitrator ("
                 << this << ") next sequence number = " << m_seqNum << ".");
...
void
McpttOnNetworkFloorArbitrator::SetStateChangeCb(
