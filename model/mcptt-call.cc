...
McpttCall::OpenFloorChannel(const Address& peerAddr, const uint16_t port)
{
    ...
    NS_ABORT_MSG_UNLESS(result == 0,
                        "Unable to open floor channel on node " << GetOwner()->GetNode()->GetId());
}
...
McpttCall::OpenMediaChannel(const Address& peerAddr, const uint16_t port)
{
    ...
    NS_ABORT_MSG_UNLESS(result == 0,
                        "Unable to open media channel on node " << GetOwner()->GetNode()->GetId());
}

void
McpttCall::ReceiveSipMessage(Ptr<Packet> pkt,
                             const sip::SipHeader& hdr,
                             sip::SipAgent::TransactionState state)
{
...
McpttCall::SendSipMessage(Ptr<Packet> pkt, const Address& addr, const sip::SipHeader& hdr)
{
    ...
    if (Ipv4Address::IsMatchingType(m_peerAddress))
    {
        GetCallChannel()->SendTo(
            pkt,
            0,
            InetSocketAddress(Ipv4Address::ConvertFrom(m_peerAddress), m_callPort));
    ...
    else if (Ipv6Address::IsMatchingType(m_peerAddress))
    {
        GetCallChannel()->SendTo(
            pkt,
            0,
            Inet6SocketAddress(Ipv6Address::ConvertFrom(m_peerAddress), m_callPort));
        GetOwner()->TraceMessageSend(GetCallId(), pkt, hdr.GetInstanceTypeId());
    }
    else
    {
...
McpttCall::ReceiveFloorPkt(Ptr<Packet> pkt, Address from)
{
    ...
    else if (subtype == McpttFloorMsgQueuePositionRequest::SUBTYPE)
    {
        ...
        GetOwner()->TraceMessageReceive(GetCallId(),
                                        pkt,
                                        queuePositionRequestMsg.GetInstanceTypeId());
...
std::ostream&
operator<<(std::ostream& os, const McpttCall::NetworkCallType& obj)
{
    os << static_cast<std::underlying_type_t<McpttCall::NetworkCallType>>(obj);
    return os;
}
