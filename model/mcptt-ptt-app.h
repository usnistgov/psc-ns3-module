...
class McpttPttApp : public Application, public McpttMediaSink
{
  public:
     ...
     */
    virtual void AcceptFloorGrant();
    /**
     * Begins an emergency alert.
     */
    virtual void BeginEmergAlert();
    /**
     * Cancels an emergency alert.
     */
    virtual void CancelEmergAlert();
    /**
     ...
     */
    virtual Ptr<McpttCall> CreateCall(ObjectFactory& callFac,
     ...
     */
    virtual void SessionInitiateRequest();
    /**
     ...
     */
    virtual void SessionReleaseRequest();
    /**
     ...
     */
    virtual void TakePushNotification();
    /**
     * Notifies the application that the button has been released.
     */
    virtual void TakeReleaseNotification();
    /**
     * Takes a request to send a media message.
     * \param msg The message to send.
     * \returns True, if the message was sent.
     */
    bool TakeSendReq(McpttMediaMsg& msg) override;
    /**
  ...
  protected:
     ...
     */
    typedef void (*TxRxTracedCallback)(Ptr<const Application> app,
                                       uint16_t callId,
     ...
     */
    typedef void (*EventTracedCallback)(uint32_t,
                                        uint16_t callId,
  ...
  private:
    ...
    std::map<Ptr<McpttChannel>, uint32_t>
        m_callChannelReferenceCount;                      //!< Reference count for call channels
    ...
    bool m_pushOnStart;        //!< The flag that indicates if the pusher should be started when the
                               //!< application starts.
    Ptr<McpttPusher> m_pusher; //!< The object that calls the Pushed() function.
    TracedCallback<Ptr<const Application>, uint16_t, Ptr<const Packet>, const TypeId&>
        m_rxTrace;                 //!< The Rx trace
    Ptr<McpttCall> m_selectedCall; //!< The currently selected call.
    Callback<void, Ptr<McpttCall>, Ptr<McpttCall>>
        m_selectedCallChangeCb; //!< The selected call change CB.
    uint32_t m_userId;          //!< The MCPTT user ID.
    TracedCallback<Ptr<const Application>, uint16_t, Ptr<const Packet>, const TypeId&>
        m_txTrace; //!< The Tx trace.
    TracedCallback<uint32_t, uint16_t, const std::string&, const char*>
        m_eventTrace; //!< Event trace
  ...
  public:
     ...
     */
    virtual std::map<uint16_t, Ptr<McpttCall>> GetCalls() const;
    /**
     ...
     */
    virtual Ptr<McpttMediaSrc> GetMediaSrc() const;
    /**
     * Gets the pusher.
     * \returns The pusher.
     */
    virtual Ptr<McpttPusher> GetPusher() const;
    /**
     * Gets the currently selected call.
     * \returns The call.
     */
    virtual Ptr<McpttCall> GetSelectedCall() const;
    /**
     ...
     */
    virtual void SetSelectedCallChangeCb(
