...
{
    static TypeId tid =
        TypeId("ns3::psc::UavMobilityEnergyModel")
            ...
            .AddTraceSource(
                ...
                "it is depleted",
                MakeTraceSourceAccessor(&UavMobilityEnergyModel::m_energyDepletionTrace),
                "ns3::UavMobilityEnergyModel::BasicCallback")
            .AddTraceSource(
                "EnergyRecharged",
                "The attached energy source has indicated that "
                "it has been recharged",
                MakeTraceSourceAccessor(&UavMobilityEnergyModel::m_energyRechargedTrace),
                "ns3::UavMobilityEnergyModel::BasicCallback")
            .AddTraceSource(
                "TotalEnergyConsumption",
                "The total energy consumed by this model (in Joules)",
                MakeTraceSourceAccessor(&UavMobilityEnergyModel::m_totalEnergyConsumption),
                "ns3::TracedValueCallback::Double")
            .AddTraceSource("State",
                            "The current mobility state determined from the "
                            " attached Mobility Model, along with the new velocity",
                            MakeTraceSourceAccessor(&UavMobilityEnergyModel::m_stateChangedTrace),
            ...
            .AddTraceSource("Current",
                            ...
                            MakeTraceSourceAccessor(&UavMobilityEnergyModel::m_currentChangedTrace),
...
UavMobilityEnergyModel::GetTotalEnergyConsumption() const
{
    ...
    double deltaEnergy = deltaUpdate.GetSeconds() * m_current * m_source->GetSupplyVoltage();
...
UavMobilityEnergyModel::Move(const Vector& velocity)
{
    ...
    double moveMagnitude = std::sqrt(velocity.x * velocity.x + velocity.y * velocity.y);
...
void
UavMobilityEnergyModel::ConnectMobility(Ptr<Node> node)
{
    NS_LOG_FUNCTION(this << node);
    m_mobility = node->GetObject<MobilityModel>();
    NS_ABORT_MSG_UNLESS(m_mobility, "No MobilityModel attached to node");

    m_mobility->TraceConnectWithoutContext(
        "CourseChange",
...
UavMobilityEnergyModel::CourseChangeCallback(Ptr<const MobilityModel> model)
{
    ...
    Vector speed = {std::abs(velocity.x), std::abs(velocity.y), std::abs(velocity.z)};
