...
class McpttFloorMsgFieldPriority : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldPriority() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
...
class McpttFloorMsgFieldDuration : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldDuration() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
 ...
 *
 * Reject Causes:
 * Cause #1 - Another MCPTT client has permission; The <b>Reject cause</b> value set to 1 indicates
 * that another MCPTT user has permission to send a media. Cause #2 - Internal floor control server
 * error; The <b>Reject cause</b> value set to 2 indicates that the floor control server cannot
 * grant the floor request due to an internal error. Cause #3 - Only one participant;    The
 * <b>Reject cause</b> value set to 3 indicates that the floor control server cannot grant the floor
 * request, because the requesting party is the only participant in the MCPTT session. Cause #4 -
 * Retry-after timer has not expired; The <b>Reject cause</b> value set to 4 indicates that the
 * floor control server cannot grant the floor request, because timer T9 (Retry-after) has not
 * expired after permission to send media has been revoked. Cause #5 - Receive only; The <b>Reject
 * cause</b> value set to 5 indicates that the floor control server cannot grant the floor request,
 * because the requesting party only has receive privilege. Cause #6 - No resources available; The
 * <b>Reject cause</b> value set to 6 indicates that the floor control server cannot grant the floor
 * request due to congestion. Cause #7  Queue full; The <b>Reject cause</b> value set to 7 indicates
 * that the floor control server cannot queue the floor request, because the queue is full. Cause
 * #255 - Other reason; The <b>Reject cause</b> value set to 255 indicates that the floor control
 * server does not grant the floor request due to the floor control server local policy.
 *
...
class McpttFloorMsgFieldRejectCause : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldRejectCause() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
  ...
  public:
     ...
     */
    virtual uint16_t GetCause() const;
    /**
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Queue Info length</b> value is a binary value and shall have the value 2 indicating the
 * total length in octets of the <b>Queue position info</b> value and the <b>Queue Priority
 * Level</b> value items. The <b>Queue Position Info</b> value is a binary value. It shall have
 * value 254 if the MCPTT client is not queued. It shall have the max value (255) if the MCPTT
 * client is queued but the MCPTT server is unable to determine the queue position or if MCPTT
 * server policy is not to release information of the queue position to the MCPTT client. The
 * <b>Queue Priority Level</b> value is coded as the <b>Floor Priority</b> value
 */
class McpttFloorMsgFieldQueuePositionInfo : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldQueuePositionInfo() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
  ...
  public:
     ...
     * \returns The value.
     */
    virtual uint8_t GetPosition() const;
    /**
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Granted Party's Identity</b> value is coded as the <b>User ID</b> value in
 * subclause 8.2.3.8. If the length of the <b>Granted Party's Identity</b> value is not a multiple
 * of 4 bytes, the Granted Party's Identity field shall be padded to a multiple of 4 bytes. The
 * value of the padding bytes should be set to zero. The padding bytes shall be ignored.
 */
class McpttFloorMsgFieldGrantedPartyId : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldGrantedPartyId() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
  ...
  public:
     ...
     */
    virtual uint32_t GetPartyId() const;
    /**
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Permission to Request the Floor length</b> value is a binary value and shall have the
 * value 2 indicating the total length in octets of the <b>Permission to Request the Floor</b> value
 * item. The <b>Permission to Request the Floor</b> value is binary and coded as follows: 0    The
 * receiver is not permitted to request floor. 1    The receiver is permitted to request floor.
 */
class McpttFloorMsgFieldPermToReq : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldPermToReq() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
  ...
  public:
     ...
     */
    virtual uint16_t GetPermission() const;
    /**
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>User ID length</b> value is a binary value and shall include the value indicating the
 * length in octets of the <b>User ID</b> value item except padding. If the length of the <b>User
 * ID</b> value is not a multiple of 4 bytes User ID field shall be padded to a multiple of 4 bytes.
 * The value of the padding bytes should be set to zero. The floor control client shall ignore the
...
class McpttFloorMsgFieldUserId : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldUserId() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Queue Size length</b> value is a binary value and shall have the value 2 indicating the
 * total length in octets of the <b>Queue Size</b> value item. The <b>Queue Size</b> value is a
 * binary value.
 */
class McpttFloorMsgFieldQueueSize : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldQueueSize() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
  ...
  public:
     ...
     */
    virtual uint16_t GetQueueSize() const;
    /**
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Message Sequence Number length</b> value is a binary value and shall have the value 2
 * indicating the total length in octets of the <b>Message Sequence Number</b> value item. The
 * <b>Message Sequence Number</b> value is a binary value. The <b>Message Sequence Number</b> value
 * can be between 0 and 65535. When the 65535 value is reached, <b>Message Sequence Number</b> value
 * starts from 0 again
 */
class McpttFloorMsgFieldSeqNum : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldSeqNum() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Queued User ID length</b> value is coded as the <b>User ID length</b> value in
 * subclause 8.2.3.11. The <b>Queued User ID</b> value is coded as the <b>User ID</b> value in
 * subclause 8.2.3.11. If the length of the <b>Queue User ID</b> value is not a multiple of 4 bytes,
 * the Queue User ID field shall be padded to a multiple of 4 bytes. The value of the padding bytes
...
class McpttFloorMsgFieldQueuedUserId : public McpttFloorMsgFieldUserId
{
  public:
     ...
     */
    ~McpttFloorMsgFieldQueuedUserId() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Source length</b> value is a binary value and shall have the value 2 indicating the total
 * length in octets of the <b>Source</b> value item. The <b>Source</b> value is a 16 bit binary
 * value where: 0    the floor participant is the source 1    the participating MCPTT function is
 * the source 2    the controlling MCPTT function is the source 3    the non-controlling MCPTT
 * function is the source All other values are reserved for future use.
 */
class McpttFloorMsgFieldSource : public McpttFloorMsgField
{
  public:
    ...
    static const uint16_t
        PARTICIPATING_FUNCTION;                 //!< The participating MCPTT function source value.
    static const uint16_t CONTROLLING_FUNCTION; //!< The controlling MCPTT function source value.
    static const uint16_t
        NONCONTROLLING_FUNCTION; //!< The non-controlling MCPTT function source value.
    /**
     ...
     */
    ~McpttFloorMsgFieldSource() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
  ...
  public:
     ...
     */
    virtual uint16_t GetSource() const;
    /**
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The <b>Track Info length</b> value is a binary value and shall have a value indicating the total
 * length in octets of the <b>Queueing Capability</b> value, <b>Track Info Priority Level</b> value
 * and one or more <b>Floor Participant Reference</b> value items. The <b>Queueing Capability</b>
 * value is an 8 bit binary value where: 0    the floor participant in the MCPTT client do not
 * support queueing 1    the floor participant in the MCPTT client do not support queueing All other
 * values are reserved for future use. The spare bits are reserved for future use and shall be set
 * to zero. The spare bits shall be ignored. The <b>Participant Type Length</b> value is 8 bit
 * binary value set to the length of the <b>Participant Type</b> value. If the length of the
 * <b>Participant</b> value is not a multiple of 4 bytes, the Track Info field shall be padded to a
 * multiple of 4 bytes. The value of the padding bytes should be set to zero. The padding bytes
 * shall be ignored. NOTE 1: The content of the <b>Participant Type</b> value is MCPTT service
 * provider specific and out of scope of the present document. All other values are reserved for
 * future use. The <b>Floor Participant Reference</b> value is a 32 bit binary value containing a
 * reference to the floor participant in the non-Controlling function of an MCPTT group. NOTE 2: The
 * reference to the floor participant is a value only understandable by the floor control server
 * interface in the non-Controlling function of an MCPTT group.
 */
class McpttFloorMsgFieldTrackInfo : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldTrackInfo() override;
    /**
     ...
     */
    virtual uint8_t GetPtLengthWithPadding() const;
    /**
     * Gets the size (in bytes) of the field when it is serialized.
     * \returns The size (in bytes).
     */
 ...
 * The <b>Message Type</b> value is an 8 bit binary value containing the binary value of the message
 * type as coded in table 8.2.3.1-2.
 */
class McpttFloorMsgFieldType : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldType() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
 ...
 *
 * \note  The indicators C, D and E are only informative. There are no procedures specified for the
 * C, D and E indicators in this release of the present document and the use of the indicators are
 * implementation specific. Bits F to P are reserved for future use and shall be set to 0. There can
 * be more than one bit set to 1 at the same time. The local policy in the floor control server
 * decides which combinations are possible and the priority of the indications.
 */
class McpttFloorMsgFieldIndic : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldIndic() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
...
class McpttFloorMsgFieldSsrc : public McpttFloorMsgField
{
  public:
     ...
     */
    ~McpttFloorMsgFieldSsrc() override;
    /**
     * Reads the contents of the field from the byte stream.
     * \param buff The buffer to read from.
     * \returns The number of bytes that were read.
