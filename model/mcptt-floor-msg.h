...
class McpttFloorMsg : public McpttMsg
{
  ...
  protected:
     ...
     */
    virtual void UpdateLength(const McpttFloorMsgField& currField,
                              const McpttFloorMsgField& newField);
    /**
 ...
 * Duration: The Duration field is only applicable in on-network and shall be coded as specified in
 * subclause 8.2.3.3. SSRC of granted floor participant: The SSRC of granted floor participant is
 * only applicable in off-network. Floor Priority: The Floor Priority field contains the granted
 * floor priority and shall be coded as specified in subclause 8.2.3.2. User ID: The User ID field
 * is used in off-network only. It shall carry the MCPTT ID of the floor participant granted the
 * floor. The User ID field shall be coded as described in subclause 8.2.3.8. Queue Size: The Queue
 * Size field is only applicable in off-network and contains the numbers of waiting MCPTT clients in
 * the MCPTT call. The Queue Size field shall be coded as specified in subclause 8.2.3.9. For each
 * waiting floor participant the following set of fields are included: 1.SSRC of queued floor
 * participant; 2.Queued User ID field; and 3.Queue info field. The set shall occur as many times as
 * the <b>Queue size</b> value in the Queue size field. SSRC of queued floor participant: The SSRC
 * of queued floor participant is only applicable in off-network and shall carry the SSRC of the
 * floor participant in the queue. Queued User ID: The Queued User ID field is only applicable in
 * off-network and contains the MCPTT ID of the floor participant in the queue. Queue Info: The
 * Queue Info field is only applicable in off-network and defines the queue position and granted
 * floor priority in the queue. Track Info: The Track Info field shall be included when an MCPTT
 * call involves a non-Controlling function. The coding of the Track Info field is described in
 * subclause 8.2.3.13. Floor Indicator: The Floor Indicator field shall be coded as described in
 * subclause 8.2.3.15.
 */
 ...
 * Length: The length shall be coded as specified in to subclause 8.1.2.
 * SSRC: The SSRC field shall carry the SSRC of the floor control server.
 * Reject Cause: The Reject Cause field includes the reason for the rejecting the floor request and
 * can be followed by a text-string explaining why the floor request was rejected. Therefore the
 * length of the packet will vary depending on the size of the application dependent field. User ID:
 * The User ID field is used in off-network only. It shall carry the MCPTT ID of the floor
 * participant sending Floor Deny message. Track Info: The Track Info field shall be included when
 * an MCPTT call involves a non-Controlling function. The coding of the Track Info field is
 * described in subclause 8.2.3.13.
 */
 ...
 * User ID: The User ID field is used in off-network only. It shall carry the MCPTT ID of the floor
 * participant sending the floor release message. Track Info: The Track Info field shall be included
 * when an MCPTT call involves a non-Controlling function. Floor Indicator: The Floor Indicator
 * field shall be coded as described in subclause 8.2.3.15.
 */
 ...
 * Reject Cause: The Reject Cause field includes the reason for the rejecting the floor request and
 * can be followed by a text-string explaining why the floor request was rejected. Therefore the
 * length of the packet will vary depending on the size of the application dependent field. Track
 * Info: The Track Info field shall be included when an MCPTT call involves a non-Controlling
 * function. The coding of the Track Info field is described in subclause 8.2.3.13.
 */
 ...
 * Length: The length shall be coded as specified in to subclause 8.1.2.
 * SSRC: The SSRC field shall carry the SSRC of the floor control server.
 * Message Sequence Number: The Message Sequence Number field shall be coded as specified in to
 * subclause 8.2.3.10. Track Info: The Track Info field shall be included when an MCPTT call
 * involves a non-Controlling function. The coding of the Track Info field is described in
 * subclause 8.2.3.13. Floor Indicator: The Floor Indicator field shall be coded as described in
 * subclause 8.2.3.15.
 */
 ...
 * Length: The length shall be coded as specified in to subclause 8.1.2.
 * SSRC: The SSRC field shall carry the SSRC of the floor control server.
 * Granted Party’s Identity: The Granted Party’s Identity field shall be coded as specified in
 * subclause 8.2.3.6. Permission to request the floor: The Permission to Request the Floor field
 * shall be coded as specified in subclause 8.2.3.7. User ID: The User ID field is used in
 * off-network only. It shall carry the MCPTT user ID of the floor participant sending the Floor
 * Taken message. Message Sequence Number: The Message Sequence Number field shall be coded as
 * specified in to subclause 8.2.3.10. Track Info: The Track Info field shall be included when an
 * MCPTT call involves a non-Controlling function. The coding of the Track Info field is described
 * in subclause 8.2.3.13. Floor Indicator: The Floor Indicator field shall be coded as described in
 * subclause 8.2.3.15.
 */
 ...
 * |                       Track Info field                        |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * SSRC: The SSRC field shall carry the SSRC of the floor participant that is requesting information
 * about its position in the floor request queue. User ID: The User ID field is used in off-network
 * only. It shall carry the MCPTT user ID of the floor participant sending the Floor Queue Position
 * Request message. Track Info: The Track Info field shall be included when an MCPTT call involves a
 * non-Controlling function.
 */
 ...
 *
 * User ID: The User ID field is used in off-network only. It shall carry the MCPTT ID of the floor
 * participant sending the Floor Queue Position Info message. SSRC of queued floor participant: The
 * SSRC of queued floor participant is only applicable in off-network and shall carry the SSRC of
 * the queued floor participant. Queued User ID: The Queued User ID field is used in off-network
 * only. It shall carry the MCPTT ID of the queued floor participant. Queue Info: The Queue Info
 * field is only applicable in off-network and defines the queue position and granted floor priority
 * in the queue. Track Info: The Track Info field shall be included when an MCPTT call involves a
 * non-Controlling function.
 */
class McpttFloorMsgQueuePositionInfo : public McpttFloorMsg
{
  ...
  private:
    McpttFloorMsgFieldUserId
        m_userId;          //!< The MCPTT ID of the floor participant that sent this message.
    uint32_t m_queuedSsrc; //!< The SSRC of the queued floor participant.
    McpttFloorMsgFieldQueuedUserId
        m_queuedUserId; //!< The MCPTT ID of the queued floor participant.
    McpttFloorMsgFieldQueuePositionInfo
        m_queueInfo; //!< Defines the queue position and granted floor priority.
    McpttFloorMsgFieldTrackInfo m_trackInfo; //!< The track info field.
    McpttFloorMsgFieldIndic m_indicator;     //!< The floor indicator field.
 ...
 * Track Info: The Track Info field shall be included when an MCPTT call involves a non-Controlling
 * function. The coding of the Track Info field is described in subclause 8.2.3.13. Floor Indicator:
 * The Floor Indicator field shall be coded as described in subclause 8.2.3.15.
 */
